...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_server_type0 = 1;
          char * server_type = (char *) malloc(_len_server_type0*sizeof(char));
          for(int _i0 = 0; _i0 < _len_server_type0; _i0++) {
            server_type[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = GetProxyServerNameAndPortFromIeProxyRegStr(name,name_size,port,str,server_type);
          printf("%d\n", benchRet); 
          free(name);
          free(port);
          free(str);

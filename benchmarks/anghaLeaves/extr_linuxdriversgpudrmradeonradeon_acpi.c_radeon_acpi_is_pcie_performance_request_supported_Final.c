...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_rdev0; _i0++) {
            rdev[_i0].atcs.functions.pcie_dev_rdy = ((-2 * (next_i()%2)) + 1) * next_i();
        rdev[_i0].atcs.functions.pcie_perf_req = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = radeon_acpi_is_pcie_performance_request_supported(rdev);
          printf("%d\n", benchRet); 
          free(rdev);

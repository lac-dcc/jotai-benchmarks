...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_adev0; _i0++) {
            adev->gmc.visible_vram_size = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_node0 = 1;
          struct drm_mm_node * node = (struct drm_mm_node *) malloc(_len_node0*sizeof(struct drm_mm_node));
          for(int _i0 = 0; _i0 < _len_node0; _i0++) {
            node->start = ((-2 * (next_i()%2)) + 1) * next_i();
        node->size = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          long benchRet = amdgpu_vram_mgr_vis_size(adev,node);
          printf("%ld\n", benchRet); 
          free(adev);
          free(node);

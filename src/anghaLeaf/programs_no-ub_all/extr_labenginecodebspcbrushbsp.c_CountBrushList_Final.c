...
struct TYPE_3__ *_allocate_brushes(int length) {
...
void _delete_brushes(struct TYPE_3__ * head) {
  if(head->next != NULL) _delete_brushes(head->next);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_3__ * brushes = _allocate_brushes(1);
          int benchRet = CountBrushList(brushes);
          printf("%d\n", benchRet); 
          _delete_brushes(brushes);

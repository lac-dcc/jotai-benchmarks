...
/* Type definitions */
struct TYPE_4__ {int fsWinProperties; int /*<<< orphan*/  fNotExpanded; } ;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info[_i0].WinType.fsWinProperties = ((-2 * (next_i()%2)) + 1) * next_i();
        info[_i0].WinType.fNotExpanded = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = navigation_visible(info);
          printf("%d\n", benchRet); 
          free(info);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_t0; _i0++) {
            t[_i0].Send.UnicastCount = ((-2 * (next_i()%2)) + 1) * next_i();
        t[_i0].Send.BroadcastCount = ((-2 * (next_i()%2)) + 1) * next_i();
        t[_i0].Recv.UnicastCount = ((-2 * (next_i()%2)) + 1) * next_i();
        t[_i0].Recv.BroadcastCount = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          long benchRet = GetTrafficPacketNum(t);
          printf("%ld\n", benchRet); 
          free(t);

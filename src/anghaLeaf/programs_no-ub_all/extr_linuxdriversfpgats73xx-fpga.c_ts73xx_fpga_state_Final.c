...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_mgr0 = 1;
          struct fpga_manager * mgr = (struct fpga_manager *) malloc(_len_mgr0*sizeof(struct fpga_manager));
          for(int _i0 = 0; _i0 < _len_mgr0; _i0++) {
            mgr->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          enum fpga_mgr_states benchRet = ts73xx_fpga_state(mgr);
          printf("{{other_type}} %p\n", &benchRet); 
          free(mgr);

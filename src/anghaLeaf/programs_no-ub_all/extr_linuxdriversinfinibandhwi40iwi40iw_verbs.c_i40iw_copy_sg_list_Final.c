...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_sg_list0; _i0++) {
            sg_list->stag = ((-2 * (next_i()%2)) + 1) * next_i();
        sg_list->len = ((-2 * (next_i()%2)) + 1) * next_i();
        sg_list->tag_off = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_sgl0 = 1;
          struct ib_sge * sgl = (struct ib_sge *) malloc(_len_sgl0*sizeof(struct ib_sge));
          for(int _i0 = 0; _i0 < _len_sgl0; _i0++) {
            sgl->lkey = ((-2 * (next_i()%2)) + 1) * next_i();
        sgl->length = ((-2 * (next_i()%2)) + 1) * next_i();
        sgl->addr = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_alloc_ctxs0 = 65025;
          struct device ** alloc_ctxs = (struct device **) malloc(_len_alloc_ctxs0*sizeof(struct device *));
          for(int _i0 = 0; _i0 < _len_alloc_ctxs0; _i0++) {
            int _len_alloc_ctxs1 = 1;
            alloc_ctxs[_i0] = (struct device *) malloc(_len_alloc_ctxs1*sizeof(struct device));
            for(int _i1 = 0; _i1 < _len_alloc_ctxs1; _i1++) {
                alloc_ctxs[_i0][_i1].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
        
            }
          }
        
          int benchRet = tw5864_queue_setup(q,num_buffers,num_planes,sizes,alloc_ctxs);
          printf("%d\n", benchRet); 
          free(q);
          free(num_buffers);
          free(num_planes);
          free(sizes);
          for(int i1 = 0; i1 < _len_alloc_ctxs0; i1++) {
              free(alloc_ctxs[i1]);
          }
          free(alloc_ctxs);

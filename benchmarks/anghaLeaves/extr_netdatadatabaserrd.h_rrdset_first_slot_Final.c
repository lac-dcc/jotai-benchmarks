...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_st0; _i0++) {
            st[_i0].counter = ((-2 * (next_i()%2)) + 1) * next_i();
        st[_i0].current_entry = ((-2 * (next_i()%2)) + 1) * next_i();
        st[_i0].entries = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned long benchRet = rrdset_first_slot(st);
          printf("%lu\n", benchRet); 
          free(st);

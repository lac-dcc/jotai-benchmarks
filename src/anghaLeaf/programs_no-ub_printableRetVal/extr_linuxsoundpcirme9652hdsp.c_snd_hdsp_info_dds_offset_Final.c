...
__attribute__((used)) static int snd_hdsp_info_dds_offset(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        uinfo->type = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = snd_hdsp_info_dds_offset(kcontrol,uinfo);
          printf("%d\n", benchRet); 
          free(kcontrol);
          free(uinfo);

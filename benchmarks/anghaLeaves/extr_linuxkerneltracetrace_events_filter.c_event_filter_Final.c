...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_file0; _i0++) {
              int _len_file__i0__filter0 = 1;
          file[_i0].filter = (struct event_filter *) malloc(_len_file__i0__filter0*sizeof(struct event_filter));
          for(int _j0 = 0; _j0 < _len_file__i0__filter0; _j0++) {
            file[_i0].filter->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct event_filter * benchRet = event_filter(file);
          printf("%d\n", (*benchRet).dummy);
          for(int _aux = 0; _aux < _len_file0; _aux++) {
          free(file[_aux].filter);
          }
          free(file);

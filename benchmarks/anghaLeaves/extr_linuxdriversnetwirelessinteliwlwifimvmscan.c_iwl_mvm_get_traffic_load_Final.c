...
__attribute__((used)) static enum iwl_mvm_traffic_load iwl_mvm_get_traffic_load(struct iwl_mvm *mvm)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_mvm0; _i0++) {
            mvm[_i0].tcm.result.global_load = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          enum iwl_mvm_traffic_load benchRet = iwl_mvm_get_traffic_load(mvm);
          printf("{{other_type}} %p\n", &benchRet); 
          free(mvm);

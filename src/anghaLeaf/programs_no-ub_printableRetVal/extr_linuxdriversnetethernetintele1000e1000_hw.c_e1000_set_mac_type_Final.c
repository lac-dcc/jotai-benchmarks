...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        hw->mac_type = ((-2 * (next_i()%2)) + 1) * next_i();
        hw->asf_firmware_present = ((-2 * (next_i()%2)) + 1) * next_i();
        hw->bad_tx_carr_stats_fd = ((-2 * (next_i()%2)) + 1) * next_i();
        hw->has_smbus = ((-2 * (next_i()%2)) + 1) * next_i();
        hw->revision_id = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          port[_i0].dev = (struct net_device *) malloc(_len_port__i0__dev0*sizeof(struct net_device));
          for(int _j0 = 0; _j0 < _len_port__i0__dev0; _j0++) {
            port[_i0].dev->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct net_device * benchRet = port_to_dev(port);
          printf("%d\n", (*benchRet).dummy);

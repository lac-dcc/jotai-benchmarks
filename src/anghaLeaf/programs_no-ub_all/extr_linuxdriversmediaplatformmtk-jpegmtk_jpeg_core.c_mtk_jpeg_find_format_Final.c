...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          unsigned int fmt_type = 255;
          int _len_ctx0 = 1;
          struct mtk_jpeg_ctx * ctx = (struct mtk_jpeg_ctx *) malloc(_len_ctx0*sizeof(struct mtk_jpeg_ctx));
          for(int _i0 = 0; _i0 < _len_ctx0; _i0++) {
            ctx->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct mtk_jpeg_fmt * benchRet = mtk_jpeg_find_format(ctx,pixelformat,fmt_type);
          printf("{{struct}} %p\n", &benchRet); 
          free(ctx);

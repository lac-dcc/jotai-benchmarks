...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_gctx__i0__ctx0; _j0++) {
              int _len_gctx__i0__ctx_authenc0 = 1;
          gctx[_i0].ctx->authenc = (struct chcr_authenc_ctx *) malloc(_len_gctx__i0__ctx_authenc0*sizeof(struct chcr_authenc_ctx));
          for(int _j0 = 0; _j0 < _len_gctx__i0__ctx_authenc0; _j0++) {
            gctx[_i0].ctx->authenc->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          struct chcr_authenc_ctx * benchRet = AUTHENC_CTX(gctx);
          printf("%d\n", (*benchRet).dummy);

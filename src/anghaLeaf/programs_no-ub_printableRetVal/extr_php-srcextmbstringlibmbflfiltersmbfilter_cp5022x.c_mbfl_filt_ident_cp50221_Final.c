...
__attribute__((used)) static int mbfl_filt_ident_cp50221(int c, mbfl_identify_filter *filter)
{
retry:
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_filter0; _i0++) {
            filter->status = ((-2 * (next_i()%2)) + 1) * next_i();
        filter->flag = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = mbfl_filt_ident_cp50221(c,filter);
          printf("%d\n", benchRet); 
          free(filter);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_sta_params0; _i0++) {
            sta_params->ht_capable = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->tx_channel_width_set = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->lsig_txop_protection = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->max_ampdu_size = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->max_ampdu_density = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->sgi_20Mhz = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->sgi_40mhz = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->green_field_capable = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->dsss_cck_mode_40mhz = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->delayed_ba_support = ((-2 * (next_i()%2)) + 1) * next_i();
        sta_params->max_amsdu_size = ((-2 * (next_i()%2)) + 1) * next_i();
          }

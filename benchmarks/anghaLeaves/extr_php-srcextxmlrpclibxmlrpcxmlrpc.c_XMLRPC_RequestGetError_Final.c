...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_request0; _i0++) {
              int _len_request__i0__error0 = 1;
          request[_i0].error = (int *) malloc(_len_request__i0__error0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_request__i0__error0; _j0++) {
            request[_i0].error[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int * benchRet = XMLRPC_RequestGetError(request);
          printf("%d\n", (*benchRet)); 
          for(int _aux = 0; _aux < _len_request0; _aux++) {
          free(request[_aux].error);
          }
          free(request);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_padapter0; _i0++) {
            padapter->driver_vcs_en = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->registrypriv.rts_thresh = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->registrypriv.wifi_spec = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->securitypriv.dot11PrivacyAlgrthm = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->xmitpriv.frag_len = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->mlmeextpriv.cur_wireless_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->mlmeextpriv.cur_bwmode = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->mlmeextpriv.mlmext_info.assoc_AP_vendor = ((-2 * (next_i()%2)) + 1) * next_i();
        padapter->mlmeextpriv.mlmext_info.HT_protection = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          ...
          for(int _i0 = 0; _i0 < _len_pxmitframe0; _i0++) {
            pxmitframe->attrib.nr_frags = ((-2 * (next_i()%2)) + 1) * next_i();
        pxmitframe->attrib.last_txcmdsz = ((-2 * (next_i()%2)) + 1) * next_i();
        pxmitframe->attrib.ampdu_en = ((-2 * (next_i()%2)) + 1) * next_i();
        pxmitframe->attrib.ht_en = ((-2 * (next_i()%2)) + 1) * next_i();
        pxmitframe->attrib.cts2self = ((-2 * (next_i()%2)) + 1) * next_i();
        pxmitframe->attrib.rtsen = ((-2 * (next_i()%2)) + 1) * next_i();
          }

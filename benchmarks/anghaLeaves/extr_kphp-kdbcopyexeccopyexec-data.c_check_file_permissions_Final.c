...
__attribute__((used)) static int check_file_permissions (char *filename, struct stat *st, int level) { return 0; }
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = check_file_permissions(filename,st,level);
          printf("%d\n", benchRet); 
          free(filename);
          free(st);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        var[_i0].xoffset = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          info[_i0].par = (struct ps3fb_par *) malloc(_len_info__i0__par0*sizeof(struct ps3fb_par));
          for(int _j0 = 0; _j0 < _len_info__i0__par0; _j0++) {
            info[_i0].par->pan_offset = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = ps3fb_pan_display(var,info);
          printf("%d\n", benchRet); 
          free(var);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_dev0; _i0++) {
            dev->tx_queue_len = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->features = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->flags = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->addr_len = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->hard_header_len = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->mtu = ((-2 * (next_i()%2)) + 1) * next_i();
        dev->type = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          can_setup(dev);
          free(dev);

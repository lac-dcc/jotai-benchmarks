...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_user0 = 1;
          struct atmel_pmecc_user * user = (struct atmel_pmecc_user *) malloc(_len_user0*sizeof(struct atmel_pmecc_user));
          for(int _i0 = 0; _i0 < _len_user0; _i0++) {
            user[_i0].cache.cfg = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = get_sectorsize(user);
          printf("%d\n", benchRet); 
          free(user);

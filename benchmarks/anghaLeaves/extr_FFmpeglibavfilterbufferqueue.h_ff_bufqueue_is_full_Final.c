...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_queue0 = 1;
          struct FFBufQueue * queue = (struct FFBufQueue *) malloc(_len_queue0*sizeof(struct FFBufQueue));
          for(int _i0 = 0; _i0 < _len_queue0; _i0++) {
            queue[_i0].available = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ff_bufqueue_is_full(queue);
          printf("%d\n", benchRet); 
          free(queue);

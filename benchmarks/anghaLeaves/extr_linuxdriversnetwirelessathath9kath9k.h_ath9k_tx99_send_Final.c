...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_txctl0 = 1;
          struct ath_tx_control * txctl = (struct ath_tx_control *) malloc(_len_txctl0*sizeof(struct ath_tx_control));
          for(int _i0 = 0; _i0 < _len_txctl0; _i0++) {
            txctl[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ath9k_tx99_send(sc,skb,txctl);
          printf("%d\n", benchRet); 
          free(sc);
          free(skb);
          free(txctl);

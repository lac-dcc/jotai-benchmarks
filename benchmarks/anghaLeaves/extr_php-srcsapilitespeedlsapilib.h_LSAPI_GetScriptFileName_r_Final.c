...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_pReq0; _i0++) {
              int _len_pReq__i0__m_pScriptFile0 = 1;
          pReq[_i0].m_pScriptFile = (char *) malloc(_len_pReq__i0__m_pScriptFile0*sizeof(char));
          for(int _j0 = 0; _j0 < _len_pReq__i0__m_pScriptFile0; _j0++) {
            pReq[_i0].m_pScriptFile[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          char * benchRet = LSAPI_GetScriptFileName_r(pReq);
          printf("%c\n", ((*benchRet) %26) + 'a'); 
          for(int _aux = 0; _aux < _len_pReq0; _aux++) {
          free(pReq[_aux].m_pScriptFile);
          }
          free(pReq);

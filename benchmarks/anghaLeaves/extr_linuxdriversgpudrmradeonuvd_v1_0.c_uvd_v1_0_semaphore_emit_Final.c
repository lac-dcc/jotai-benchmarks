...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_semaphore0 = 1;
          struct radeon_semaphore * semaphore = (struct radeon_semaphore *) malloc(_len_semaphore0*sizeof(struct radeon_semaphore));
          for(int _i0 = 0; _i0 < _len_semaphore0; _i0++) {
            semaphore[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = uvd_v1_0_semaphore_emit(rdev,ring,semaphore,emit_wait);
          printf("%d\n", benchRet); 
          free(rdev);
          free(ring);
          free(semaphore);

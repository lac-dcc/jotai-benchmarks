...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_adapter0; _i0++) {
            adapter->info.vesw.eth_mtu = ((-2 * (next_i()%2)) + 1) * next_i();
        adapter->info.vport.eth_link_status = ((-2 * (next_i()%2)) + 1) * next_i();
        adapter->info.vport.config_state = ((-2 * (next_i()%2)) + 1) * next_i();
        adapter->info.vport.max_smac_ent = ((-2 * (next_i()%2)) + 1) * next_i();
        adapter->info.vport.max_mac_tbl_ent = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_SPI_InitStruct0; _i0++) {
            SPI_InitStruct->SPI_CRCPolynomial = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_FirstBit = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_BaudRatePrescaler = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_NSS = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_CPHA = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_CPOL = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_DataSize = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_Mode = ((-2 * (next_i()%2)) + 1) * next_i();
        SPI_InitStruct->SPI_Direction = ((-2 * (next_i()%2)) + 1) * next_i();
          }

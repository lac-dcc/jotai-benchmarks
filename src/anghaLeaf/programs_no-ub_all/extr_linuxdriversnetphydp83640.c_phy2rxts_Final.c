...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->sec_lo = ((-2 * (next_i()%2)) + 1) * next_i();
        p->sec_hi = ((-2 * (next_i()%2)) + 1) * next_i();
        p->ns_lo = ((-2 * (next_i()%2)) + 1) * next_i();
        p->ns_hi = ((-2 * (next_i()%2)) + 1) * next_i();
        p->msgtype = ((-2 * (next_i()%2)) + 1) * next_i();
        p->seqid = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_rxts0 = 1;
          struct rxts * rxts = (struct rxts *) malloc(_len_rxts0*sizeof(struct rxts));
          for(int _i0 = 0; _i0 < _len_rxts0; _i0++) {
            rxts->ns = ((-2 * (next_i()%2)) + 1) * next_i();
        rxts->msgtype = ((-2 * (next_i()%2)) + 1) * next_i();
        rxts->hash = ((-2 * (next_i()%2)) + 1) * next_i();
        rxts->tmo = ((-2 * (next_i()%2)) + 1) * next_i();
        rxts->seqid = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          phy2rxts(p,rxts);
          free(p);
          free(rxts);

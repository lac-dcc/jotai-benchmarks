...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_address0 = 1;
          struct sockaddr * address = (struct sockaddr *) malloc(_len_address0*sizeof(struct sockaddr));
          for(int _i0 = 0; _i0 < _len_address0; _i0++) {
            address[_i0].sa_family = ((-2 * (next_i()%2)) + 1) * next_i();
        address[_i0].sa_len = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = necp_client_address_is_valid(address);
          printf("%d\n", benchRet); 
          free(address);

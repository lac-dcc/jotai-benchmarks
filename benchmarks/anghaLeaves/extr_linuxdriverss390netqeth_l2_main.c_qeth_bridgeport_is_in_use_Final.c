...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_card0; _i0++) {
            card[_i0].options.sbp.hostnotification = ((-2 * (next_i()%2)) + 1) * next_i();
        card[_i0].options.sbp.reflect_promisc = ((-2 * (next_i()%2)) + 1) * next_i();
        card[_i0].options.sbp.role = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = qeth_bridgeport_is_in_use(card);
          printf("%d\n", benchRet); 
          free(card);

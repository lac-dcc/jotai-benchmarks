...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_n0 = 1;
          struct ulist_node * n = (struct ulist_node *) malloc(_len_n0*sizeof(struct ulist_node));
          for(int _i0 = 0; _i0 < _len_n0; _i0++) {
            n->aux = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct btrfs_qgroup * benchRet = unode_aux_to_qgroup(n);
          printf("{{struct}} %p\n", &benchRet); 
          free(n);

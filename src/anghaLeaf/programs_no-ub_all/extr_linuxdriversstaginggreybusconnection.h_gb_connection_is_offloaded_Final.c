...
/* Variables and functions */
 int GB_CONNECTION_FLAG_OFFLOADED ; 

__attribute__((used)) static inline bool gb_connection_is_offloaded(struct gb_connection *connection)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_connection0; _i0++) {
            connection->flags = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = gb_connection_is_offloaded(connection);
          printf("%d\n", benchRet); 
          free(connection);

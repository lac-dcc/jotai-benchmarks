...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        a->time.tick = ((-2 * (next_i()%2)) + 1) * next_i();
        a->time.time.tv_sec = ((-2 * (next_i()%2)) + 1) * next_i();
        a->time.time.tv_nsec = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_b0 = 1;
          struct snd_seq_event * b = (struct snd_seq_event *) malloc(_len_b0*sizeof(struct snd_seq_event));
          for(int _i0 = 0; _i0 < _len_b0; _i0++) {
            b->flags = ((-2 * (next_i()%2)) + 1) * next_i();
        b->time.tick = ((-2 * (next_i()%2)) + 1) * next_i();
        b->time.time.tv_sec = ((-2 * (next_i()%2)) + 1) * next_i();
        b->time.time.tv_nsec = ((-2 * (next_i()%2)) + 1) * next_i();
          }

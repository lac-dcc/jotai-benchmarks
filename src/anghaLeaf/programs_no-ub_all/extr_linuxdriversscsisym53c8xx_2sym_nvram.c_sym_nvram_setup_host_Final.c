...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_shost0; _i0++) {
            shost->reverse_ordering = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_np0 = 1;
          struct sym_hcb * np = (struct sym_hcb *) malloc(_len_np0*sizeof(struct sym_hcb));
          for(int _i0 = 0; _i0 < _len_np0; _i0++) {
            np->rv_scntl0 = ((-2 * (next_i()%2)) + 1) * next_i();
        np->myaddr = ((-2 * (next_i()%2)) + 1) * next_i();
        np->verbose = ((-2 * (next_i()%2)) + 1) * next_i();
        np->usrflags = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_nvram0 = 1;
          struct sym_nvram * nvram = (struct sym_nvram *) malloc(_len_nvram0*sizeof(struct sym_nvram));
          for(int _i0 = 0; _i0 < _len_nvram0; _i0++) {
            nvram->type = ((-2 * (next_i()%2)) + 1) * next_i();
        nvram->data.Tekram.host_id = ((-2 * (next_i()%2)) + 1) * next_i();
        nvram->data.Symbios.flags = ((-2 * (next_i()%2)) + 1) * next_i();
        nvram->data.Symbios.host_id = ((-2 * (next_i()%2)) + 1) * next_i();
        nvram->data.Symbios.flags1 = ((-2 * (next_i()%2)) + 1) * next_i();
        nvram->data.Symbios.flags2 = ((-2 * (next_i()%2)) + 1) * next_i();
          }

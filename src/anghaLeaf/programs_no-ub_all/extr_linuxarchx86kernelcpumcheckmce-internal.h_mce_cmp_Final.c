...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_m10; _i0++) {
            m1->bank = ((-2 * (next_i()%2)) + 1) * next_i();
        m1->status = ((-2 * (next_i()%2)) + 1) * next_i();
        m1->addr = ((-2 * (next_i()%2)) + 1) * next_i();
        m1->misc = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_m20 = 1;
          struct mce * m2 = (struct mce *) malloc(_len_m20*sizeof(struct mce));
          for(int _i0 = 0; _i0 < _len_m20; _i0++) {
            m2->bank = ((-2 * (next_i()%2)) + 1) * next_i();
        m2->status = ((-2 * (next_i()%2)) + 1) * next_i();
        m2->addr = ((-2 * (next_i()%2)) + 1) * next_i();
        m2->misc = ((-2 * (next_i()%2)) + 1) * next_i();
          }

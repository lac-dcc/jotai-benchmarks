...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_tc0 = 1;
          struct transport_container * tc = (struct transport_container *) malloc(_len_tc0*sizeof(struct transport_container));
          ...
          int _len_cdev0 = 1;
          struct device * cdev = (struct device *) malloc(_len_cdev0*sizeof(struct device));
          for(int _i0 = 0; _i0 < _len_cdev0; _i0++) {
            cdev[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = anon_transport_dummy_function(tc,dev,cdev);
          printf("%d\n", benchRet); 
          free(tc);
          free(dev);
          free(cdev);

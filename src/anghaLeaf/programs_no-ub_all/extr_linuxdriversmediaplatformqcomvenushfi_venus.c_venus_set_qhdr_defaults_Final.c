...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_qhdr0; _i0++) {
            qhdr->status = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->q_size = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->rx_wm = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->tx_wm = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->rx_req = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->write_idx = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->read_idx = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->tx_irq_status = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->rx_irq_status = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->tx_req = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->pkt_size = ((-2 * (next_i()%2)) + 1) * next_i();
        qhdr->type = ((-2 * (next_i()%2)) + 1) * next_i();
          }

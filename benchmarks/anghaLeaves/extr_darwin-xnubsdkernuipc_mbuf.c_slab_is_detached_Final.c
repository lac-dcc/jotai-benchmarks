...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_sp0; _i0++) {
            sp[_i0].sl_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        sp[_i0].sl_link.tqe_prev = ((-2 * (next_i()%2)) + 1) * next_i();
        sp[_i0].sl_link.tqe_next = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = slab_is_detached(sp);
          printf("%d\n", benchRet); 
          free(sp);

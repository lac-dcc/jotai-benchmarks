...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_lhs0 = 1;
          struct Range * lhs = (struct Range *) malloc(_len_lhs0*sizeof(struct Range));
          for(int _i0 = 0; _i0 < _len_lhs0; _i0++) {
            lhs[_i0].begin = ((-2 * (next_i()%2)) + 1) * next_i();
        lhs[_i0].end = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct Range rhs;
        rhs.begin = ((-2 * (next_i()%2)) + 1) * next_i();
        rhs.end = ((-2 * (next_i()%2)) + 1) * next_i();
          range_combine(lhs,rhs);
          free(lhs);

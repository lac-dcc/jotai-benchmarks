...
__attribute__((used)) static int dummy_get_ts_info(struct net_device *dev,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ts_info0; _i0++) {
            ts_info->so_timestamping = ((-2 * (next_i()%2)) + 1) * next_i();
        ts_info->phc_index = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = dummy_get_ts_info(dev,ts_info);
          printf("%d\n", benchRet); 
          free(dev);
          free(ts_info);

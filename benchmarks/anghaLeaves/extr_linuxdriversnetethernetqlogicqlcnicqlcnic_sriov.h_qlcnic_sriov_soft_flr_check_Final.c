...
__attribute__((used)) static inline bool qlcnic_sriov_soft_flr_check(struct qlcnic_adapter *adapter,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_vf0; _i0++) {
            vf[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = qlcnic_sriov_soft_flr_check(adapter,trans,vf);
          printf("%d\n", benchRet); 
          free(adapter);
          free(trans);
          free(vf);

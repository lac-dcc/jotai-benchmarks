...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          rproc[_i0].priv = (struct q6v5 *) malloc(_len_rproc__i0__priv0*sizeof(struct q6v5));
          for(int _j0 = 0; _j0 < _len_rproc__i0__priv0; _j0++) {
            rproc[_i0].priv->mpss_reloc = ((-2 * (next_i()%2)) + 1) * next_i();
        rproc[_i0].priv->mpss_size = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          void * benchRet = q6v5_da_to_va(rproc,da,len);
          printf("{{other_type}} %p\n", &benchRet); 

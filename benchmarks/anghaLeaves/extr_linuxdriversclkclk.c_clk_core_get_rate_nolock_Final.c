...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_core0; _i0++) {
            core[_i0].rate = ((-2 * (next_i()%2)) + 1) * next_i();
        core[_i0].parent = ((-2 * (next_i()%2)) + 1) * next_i();
        core[_i0].num_parents = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned long benchRet = clk_core_get_rate_nolock(core);
          printf("%lu\n", benchRet); 
          free(core);

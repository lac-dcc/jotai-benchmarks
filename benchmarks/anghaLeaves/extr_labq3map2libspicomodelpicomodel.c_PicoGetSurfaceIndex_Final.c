...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_surface0; _i0++) {
            surface[_i0].numIndexes = ((-2 * (next_i()%2)) + 1) * next_i();
          int _len_surface__i0__index0 = 1;
          surface[_i0].index = (int *) malloc(_len_surface__i0__index0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_surface__i0__index0; _j0++) {
            surface[_i0].index[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = PicoGetSurfaceIndex(surface,num);
          printf("%d\n", benchRet); 
          for(int _aux = 0; _aux < _len_surface0; _aux++) {
          free(surface[_aux].index);
          }
          free(surface);

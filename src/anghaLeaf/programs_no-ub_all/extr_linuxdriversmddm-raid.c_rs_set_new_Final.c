...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_rs0; _i0++) {
            rs->raid_disks = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.delta_disks = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.raid_disks = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.new_chunk_sectors = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.chunk_sectors = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.new_layout = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.layout = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.new_level = ((-2 * (next_i()%2)) + 1) * next_i();
        rs->md.level = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          rs_set_new(rs);
          free(rs);

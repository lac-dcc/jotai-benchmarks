...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_skel0; _i0++) {
            skel->input = ((-2 * (next_i()%2)) + 1) * next_i();
        skel->std = ((-2 * (next_i()%2)) + 1) * next_i();
        skel->timings.bt.width = ((-2 * (next_i()%2)) + 1) * next_i();
        skel->timings.bt.height = ((-2 * (next_i()%2)) + 1) * next_i();
        skel->timings.bt.interlaced = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_pix0 = 1;
          ...
          for(int _i0 = 0; _i0 < _len_pix0; _i0++) {
            pix->width = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->height = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->bytesperline = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->sizeimage = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->priv = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->colorspace = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->field = ((-2 * (next_i()%2)) + 1) * next_i();
        pix->pixelformat = ((-2 * (next_i()%2)) + 1) * next_i();
          }

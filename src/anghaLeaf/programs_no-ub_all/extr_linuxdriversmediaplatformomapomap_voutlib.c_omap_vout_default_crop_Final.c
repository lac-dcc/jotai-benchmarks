    ...
    case 0:
    {
        ...
        pix->height = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          for(int _i0 = 0; _i0 < _len_fbuf0; _i0++) {
            fbuf->fmt.width = ((-2 * (next_i()%2)) + 1) * next_i();
        fbuf->fmt.height = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_crop0 = 1;
          struct v4l2_rect * crop = (struct v4l2_rect *) malloc(_len_crop0*sizeof(struct v4l2_rect));
          for(int _i0 = 0; _i0 < _len_crop0; _i0++) {
            crop->width = ((-2 * (next_i()%2)) + 1) * next_i();
        crop->height = ((-2 * (next_i()%2)) + 1) * next_i();
        crop->left = ((-2 * (next_i()%2)) + 1) * next_i();
        crop->top = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_op0; _i0++) {
            op->code_rate_HP = ((-2 * (next_i()%2)) + 1) * next_i();
        op->code_rate_LP = ((-2 * (next_i()%2)) + 1) * next_i();
        op->modulation = ((-2 * (next_i()%2)) + 1) * next_i();
        op->transmission_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        op->guard_interval = ((-2 * (next_i()%2)) + 1) * next_i();
        op->hierarchy = ((-2 * (next_i()%2)) + 1) * next_i();
          }

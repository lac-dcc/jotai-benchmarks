...
__attribute__((used)) static int ff_irq_request(struct fmc_device *fmc, irq_handler_t handler,
			    char *name, int flags)
{
	return -EOPNOTSUPP;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_fmc0; _i0++) {
              fmc[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = ff_irq_request(fmc,handler,name,flags);
          printf("%d\n", benchRet); 
          free(fmc);
          free(name);
    ...
    case 3:
    {
          ...
          int handler = ((-2 * (next_i()%2)) + 1) * next_i();

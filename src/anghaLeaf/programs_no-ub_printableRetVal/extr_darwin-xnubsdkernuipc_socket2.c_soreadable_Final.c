...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_so0; _i0++) {
            so->so_state = ((-2 * (next_i()%2)) + 1) * next_i();
        so->so_error = ((-2 * (next_i()%2)) + 1) * next_i();
        so->so_comp.tqh_first = ((-2 * (next_i()%2)) + 1) * next_i();
        so->so_rcv.sb_cc = ((-2 * (next_i()%2)) + 1) * next_i();
        so->so_rcv.sb_lowat = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = soreadable(so);
          printf("%d\n", benchRet); 
          free(so);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->m_nBytesRead = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_nBodySize = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_hasAbsTimestamp = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_nInfoField2 = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_nTimeStamp = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_nChannel = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_packetType = ((-2 * (next_i()%2)) + 1) * next_i();
        p->m_headerType = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          RTMPPacket_Reset(p);
          free(p);

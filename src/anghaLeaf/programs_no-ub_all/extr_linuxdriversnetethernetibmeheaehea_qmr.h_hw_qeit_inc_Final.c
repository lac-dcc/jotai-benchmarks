...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_queue0; _i0++) {
            queue->current_q_offset = ((-2 * (next_i()%2)) + 1) * next_i();
        queue->qe_size = ((-2 * (next_i()%2)) + 1) * next_i();
        queue->queue_length = ((-2 * (next_i()%2)) + 1) * next_i();
        queue->toggle_state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          hw_qeit_inc(queue);
          free(queue);

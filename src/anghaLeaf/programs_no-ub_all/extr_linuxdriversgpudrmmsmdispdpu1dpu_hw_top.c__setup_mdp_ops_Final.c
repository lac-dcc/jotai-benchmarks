...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ops0; _i0++) {
            ops->intf_audio_select = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->reset_ubwc = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->get_safe_status = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->setup_vsync_source = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->get_danger_status = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->setup_clk_force_ctrl = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->setup_cdm_output = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->setup_split_pipe = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
__attribute__((used)) static inline int omap_vout_vrfb_buffer_setup(struct omap_vout_device *vout,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = omap_vout_vrfb_buffer_setup(vout,count,startindex);
          printf("%d\n", benchRet); 
          free(vout);
          free(count);

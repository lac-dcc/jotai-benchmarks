...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_pdu0; _i0++) {
            pdu->u.ret_submit.error_count = ((-2 * (next_i()%2)) + 1) * next_i();
        pdu->u.ret_submit.number_of_packets = ((-2 * (next_i()%2)) + 1) * next_i();
        pdu->u.ret_submit.start_frame = ((-2 * (next_i()%2)) + 1) * next_i();
        pdu->u.ret_submit.actual_length = ((-2 * (next_i()%2)) + 1) * next_i();
        pdu->u.ret_submit.status = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_urb0 = 1;
          struct urb * urb = (struct urb *) malloc(_len_urb0*sizeof(struct urb));
          for(int _i0 = 0; _i0 < _len_urb0; _i0++) {
            urb->error_count = ((-2 * (next_i()%2)) + 1) * next_i();
        urb->number_of_packets = ((-2 * (next_i()%2)) + 1) * next_i();
        urb->start_frame = ((-2 * (next_i()%2)) + 1) * next_i();
        urb->actual_length = ((-2 * (next_i()%2)) + 1) * next_i();
        urb->status = ((-2 * (next_i()%2)) + 1) * next_i();
          }

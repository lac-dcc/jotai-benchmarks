...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_stat0; _i0++) {
            stat->ca_initialization_status = ((-2 * (next_i()%2)) + 1) * next_i();
        stat->ca_error_flag = ((-2 * (next_i()%2)) + 1) * next_i();
        stat->ca_dvb_flag = ((-2 * (next_i()%2)) + 1) * next_i();
        stat->ca_module_present_status = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = fdtv_ca_ready(stat);
          printf("%d\n", benchRet); 
          free(stat);

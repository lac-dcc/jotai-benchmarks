...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_iter0; _i0++) {
            iter[_i0].skip_tree = ((-2 * (next_i()%2)) + 1) * next_i();
        iter[_i0].next_idx = ((-2 * (next_i()%2)) + 1) * next_i();
        iter[_i0].base.flags = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = index_iterator_init(iter);
          printf("%d\n", benchRet); 
          free(iter);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_cfg0; _i0++) {
            cfg->reset_on_init = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->promiscuous_mode_enable = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->pause_ignore = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->pause_quanta = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->max_frame_length = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->tx_ipg_length = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          set_dflts(cfg);
          free(cfg);

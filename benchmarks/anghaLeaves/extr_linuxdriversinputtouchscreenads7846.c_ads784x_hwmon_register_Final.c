...
__attribute__((used)) static inline int ads784x_hwmon_register(struct spi_device *spi,
					 struct ads7846 *ts)
{
	return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_ts0 = 1;
          struct ads7846 * ts = (struct ads7846 *) malloc(_len_ts0*sizeof(struct ads7846));
          for(int _i0 = 0; _i0 < _len_ts0; _i0++) {
            ts[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ads784x_hwmon_register(spi,ts);
          printf("%d\n", benchRet); 
          free(spi);
          free(ts);

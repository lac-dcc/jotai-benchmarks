...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_iter0; _i0++) {
            iter->started = ((-2 * (next_i()%2)) + 1) * next_i();
        iter->ended = ((-2 * (next_i()%2)) + 1) * next_i();
        iter->flags = ((-2 * (next_i()%2)) + 1) * next_i();
        iter->pathlist_walk_idx = ((-2 * (next_i()%2)) + 1) * next_i();
        iter->stat_calls = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          iterator_clear(iter);
          free(iter);

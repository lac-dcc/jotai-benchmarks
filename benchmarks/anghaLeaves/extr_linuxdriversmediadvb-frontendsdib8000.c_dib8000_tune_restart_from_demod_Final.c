...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          fe[_i0].demodulator_priv = (struct dib8000_state *) malloc(_len_fe__i0__demodulator_priv0*sizeof(struct dib8000_state));
          for(int _j0 = 0; _j0 < _len_fe__i0__demodulator_priv0; _j0++) {
            fe[_i0].demodulator_priv->tune_state = ((-2 * (next_i()%2)) + 1) * next_i();
        fe[_i0].demodulator_priv->status = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = dib8000_tune_restart_from_demod(fe);
          printf("%d\n", benchRet); 

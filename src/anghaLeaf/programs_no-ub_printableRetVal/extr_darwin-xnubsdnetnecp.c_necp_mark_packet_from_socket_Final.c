...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_packet0; _i0++) {
            packet->m_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        packet->m_pkthdr.necp_mtag.necp_skip_policy_id = ((-2 * (next_i()%2)) + 1) * next_i();
        packet->m_pkthdr.necp_mtag.necp_app_id = ((-2 * (next_i()%2)) + 1) * next_i();
        packet->m_pkthdr.necp_mtag.necp_route_rule_id = ((-2 * (next_i()%2)) + 1) * next_i();
        packet->m_pkthdr.necp_mtag.necp_last_interface_index = ((-2 * (next_i()%2)) + 1) * next_i();
        packet->m_pkthdr.necp_mtag.necp_policy_id = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_inp0 = 1;
          struct inpcb * inp = (struct inpcb *) malloc(_len_inp0*sizeof(struct inpcb));
          for(int _i0 = 0; _i0 < _len_inp0; _i0++) {
            inp->inp_policyresult.app_id = ((-2 * (next_i()%2)) + 1) * next_i();
        inp->inp_policyresult.results.result = ((-2 * (next_i()%2)) + 1) * next_i();
        inp->inp_policyresult.results.route_rule_id = ((-2 * (next_i()%2)) + 1) * next_i();
        inp->inp_policyresult.policy_id = ((-2 * (next_i()%2)) + 1) * next_i();
          }

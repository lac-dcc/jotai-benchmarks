...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_host0; _i0++) {
            host->rect.left = ((-2 * (next_i()%2)) + 1) * next_i();
        host->rect.right = ((-2 * (next_i()%2)) + 1) * next_i();
        host->rect.top = ((-2 * (next_i()%2)) + 1) * next_i();
        host->rect.bottom = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_ret0 = 1;
          struct TYPE_7__ * ret = (struct TYPE_7__ *) malloc(_len_ret0*sizeof(struct TYPE_7__));
          for(int _i0 = 0; _i0 < _len_ret0; _i0++) {
            ret->right = ((-2 * (next_i()%2)) + 1) * next_i();
        ret->bottom = ((-2 * (next_i()%2)) + 1) * next_i();
        ret->left = ((-2 * (next_i()%2)) + 1) * next_i();
        ret->top = ((-2 * (next_i()%2)) + 1) * next_i();
          }

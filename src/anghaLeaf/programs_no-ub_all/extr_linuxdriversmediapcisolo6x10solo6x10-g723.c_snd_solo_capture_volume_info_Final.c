...
__attribute__((used)) static int snd_solo_capture_volume_info(struct snd_kcontrol *kcontrol,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info->count = ((-2 * (next_i()%2)) + 1) * next_i();
        info->value.integer.max = ((-2 * (next_i()%2)) + 1) * next_i();
        info->value.integer.step = ((-2 * (next_i()%2)) + 1) * next_i();
        info->value.integer.min = ((-2 * (next_i()%2)) + 1) * next_i();
        info->type = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = snd_solo_capture_volume_info(kcontrol,info);
          printf("%d\n", benchRet); 
          free(kcontrol);
          free(info);

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_info0 = 1;
          struct vfio_irq_info * info = (struct vfio_irq_info *) malloc(_len_info0*sizeof(struct vfio_irq_info));
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info->index = ((-2 * (next_i()%2)) + 1) * next_i();
        info->count = ((-2 * (next_i()%2)) + 1) * next_i();
        info->flags = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = vfio_ccw_mdev_get_irq_info(info);
          printf("%d\n", benchRet); 
          free(info);

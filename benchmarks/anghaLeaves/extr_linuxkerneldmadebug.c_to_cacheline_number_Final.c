...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_entry0 = 1;
          struct dma_debug_entry * entry = (struct dma_debug_entry *) malloc(_len_entry0*sizeof(struct dma_debug_entry));
          for(int _i0 = 0; _i0 < _len_entry0; _i0++) {
            entry[_i0].pfn = ((-2 * (next_i()%2)) + 1) * next_i();
        entry[_i0].offset = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = to_cacheline_number(entry);
          printf("%d\n", benchRet); 
          free(entry);

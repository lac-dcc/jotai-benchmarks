...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_adev0; _i0++) {
            adev[_i0].atcs.functions.pcie_dev_rdy = ((-2 * (next_i()%2)) + 1) * next_i();
        adev[_i0].atcs.functions.pcie_perf_req = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = amdgpu_acpi_is_pcie_performance_request_supported(adev);
          printf("%d\n", benchRet); 
          free(adev);

...
__attribute__((used)) static inline int ceph_readpages_from_fscache(struct inode *inode,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = ceph_readpages_from_fscache(inode,mapping,pages,nr_pages);
          printf("%d\n", benchRet); 
          free(inode);
          free(mapping);
          free(pages);

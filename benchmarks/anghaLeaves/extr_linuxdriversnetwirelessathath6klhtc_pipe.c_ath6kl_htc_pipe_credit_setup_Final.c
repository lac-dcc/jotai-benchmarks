...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_info0 = 1;
          struct ath6kl_htc_credit_info * info = (struct ath6kl_htc_credit_info *) malloc(_len_info0*sizeof(struct ath6kl_htc_credit_info));
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ath6kl_htc_pipe_credit_setup(target,info);
          printf("%d\n", benchRet); 
          free(target);
          free(info);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        so[_i0].so_error = ((-2 * (next_i()%2)) + 1) * next_i();
        so[_i0].so_comp.tqh_first = ((-2 * (next_i()%2)) + 1) * next_i();
        so[_i0].so_rcv.sb_cc = ((-2 * (next_i()%2)) + 1) * next_i();
        so[_i0].so_rcv.sb_lowat = ((-2 * (next_i()%2)) + 1) * next_i();
          }

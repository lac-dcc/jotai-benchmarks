...
__attribute__((used)) static int flexonenand_ooblayout_free(struct mtd_info *mtd, int section,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_oobregion0; _i0++) {
            oobregion->offset = ((-2 * (next_i()%2)) + 1) * next_i();
        oobregion->length = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = flexonenand_ooblayout_free(mtd,section,oobregion);
          printf("%d\n", benchRet); 
          free(mtd);
          free(oobregion);

...
__attribute__((used)) static inline int qeth_cut_iqd_prio(struct qeth_card *card, int queue_num)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_card0; _i0++) {
            card->info.type = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = qeth_cut_iqd_prio(card,queue_num);
          printf("%d\n", benchRet); 
          free(card);

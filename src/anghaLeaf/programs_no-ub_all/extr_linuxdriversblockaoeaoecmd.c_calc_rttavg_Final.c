...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_d0; _i0++) {
            d->rttavg = ((-2 * (next_i()%2)) + 1) * next_i();
        d->rttdev = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_t0 = 1;
          struct aoetgt * t = (struct aoetgt *) malloc(_len_t0*sizeof(struct aoetgt));
          for(int _i0 = 0; _i0 < _len_t0; _i0++) {
            t->maxout = ((-2 * (next_i()%2)) + 1) * next_i();
        t->nframes = ((-2 * (next_i()%2)) + 1) * next_i();
        t->ssthresh = ((-2 * (next_i()%2)) + 1) * next_i();
        t->nout = ((-2 * (next_i()%2)) + 1) * next_i();
        t->next_cwnd = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          calc_rttavg(d,t,rtt);
          free(d);
          free(t);

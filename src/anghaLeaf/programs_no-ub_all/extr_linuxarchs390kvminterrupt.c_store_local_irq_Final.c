...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_li0; _i0++) {
            li->irq.prefix = ((-2 * (next_i()%2)) + 1) * next_i();
        li->irq.stop = ((-2 * (next_i()%2)) + 1) * next_i();
        li->irq.extcall = ((-2 * (next_i()%2)) + 1) * next_i();
        li->irq.ext = ((-2 * (next_i()%2)) + 1) * next_i();
        li->irq.pgm = ((-2 * (next_i()%2)) + 1) * next_i();
        li->irq.mchk = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_irq0 = 1;
          struct kvm_s390_irq * irq = (struct kvm_s390_irq *) malloc(_len_irq0*sizeof(struct kvm_s390_irq));
          for(int _i0 = 0; _i0 < _len_irq0; _i0++) {
            irq->u.prefix = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->u.stop = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->u.extcall = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->u.ext = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->u.pgm = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->u.mchk = ((-2 * (next_i()%2)) + 1) * next_i();
        irq->type = ((-2 * (next_i()%2)) + 1) * next_i();
          }

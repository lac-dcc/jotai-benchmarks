...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info->flex_ratio_max = ((-2 * (next_i()%2)) + 1) * next_i();
        info->flex_ratio_min = ((-2 * (next_i()%2)) + 1) * next_i();
        info->flex_ratio = ((-2 * (next_i()%2)) + 1) * next_i();
        info->busFreq = ((-2 * (next_i()%2)) + 1) * next_i();
        info->bus2tsc = ((-2 * (next_i()%2)) + 1) * next_i();
        info->tscGranularity = ((-2 * (next_i()%2)) + 1) * next_i();
        info->tscFCvtn2t = ((-2 * (next_i()%2)) + 1) * next_i();
        info->tscFCvtt2n = ((-2 * (next_i()%2)) + 1) * next_i();
        info->tscFreq = ((-2 * (next_i()%2)) + 1) * next_i();
        info->busFCvtn2t = ((-2 * (next_i()%2)) + 1) * next_i();
        info->busFCvtt2n = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          tsc_get_info(info);
          free(info);

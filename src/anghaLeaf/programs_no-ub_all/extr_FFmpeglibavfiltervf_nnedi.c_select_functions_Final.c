...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_s0; _i0++) {
            s->pscrn = ((-2 * (next_i()%2)) + 1) * next_i();
        s->fapprox = ((-2 * (next_i()%2)) + 1) * next_i();
        s->expfunc = ((-2 * (next_i()%2)) + 1) * next_i();
        s->dot_prod = ((-2 * (next_i()%2)) + 1) * next_i();
        s->extract = ((-2 * (next_i()%2)) + 1) * next_i();
        s->wae5 = ((-2 * (next_i()%2)) + 1) * next_i();
        s->compute_network0 = ((-2 * (next_i()%2)) + 1) * next_i();
        s->readpixels = ((-2 * (next_i()%2)) + 1) * next_i();
        s->process_line0 = ((-2 * (next_i()%2)) + 1) * next_i();
        s->evalfunc_1 = ((-2 * (next_i()%2)) + 1) * next_i();
        s->evalfunc_0 = ((-2 * (next_i()%2)) + 1) * next_i();
        s->copy_pad = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          select_functions(s);
          free(s);

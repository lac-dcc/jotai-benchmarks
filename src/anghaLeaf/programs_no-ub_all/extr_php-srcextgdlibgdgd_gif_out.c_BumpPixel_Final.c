...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ctx0; _i0++) {
            ctx->curx = ((-2 * (next_i()%2)) + 1) * next_i();
        ctx->Width = ((-2 * (next_i()%2)) + 1) * next_i();
        ctx->cury = ((-2 * (next_i()%2)) + 1) * next_i();
        ctx->Pass = ((-2 * (next_i()%2)) + 1) * next_i();
        ctx->Height = ((-2 * (next_i()%2)) + 1) * next_i();
        ctx->Interlace = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          BumpPixel(ctx);
          free(ctx);

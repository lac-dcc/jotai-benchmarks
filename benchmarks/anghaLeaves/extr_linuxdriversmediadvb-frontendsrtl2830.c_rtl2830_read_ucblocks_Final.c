...
__attribute__((used)) static int rtl2830_read_ucblocks(struct dvb_frontend *fe, u32 *ucblocks)
{
	*ucblocks = 0;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          long * ucblocks = (long *) malloc(_len_ucblocks0*sizeof(long));
          for(int _i0 = 0; _i0 < _len_ucblocks0; _i0++) {
            ucblocks[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = rtl2830_read_ucblocks(fe,ucblocks);
          printf("%d\n", benchRet); 
          free(fe);
          free(ucblocks);

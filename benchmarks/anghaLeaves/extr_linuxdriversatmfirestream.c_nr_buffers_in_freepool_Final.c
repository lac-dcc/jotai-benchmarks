...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_dev0 = 1;
          struct fs_dev * dev = (struct fs_dev *) malloc(_len_dev0*sizeof(struct fs_dev));
          for(int _i0 = 0; _i0 < _len_dev0; _i0++) {
            dev[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_fp0 = 1;
          struct freepool * fp = (struct freepool *) malloc(_len_fp0*sizeof(struct freepool));
          for(int _i0 = 0; _i0 < _len_fp0; _i0++) {
            fp[_i0].n = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = nr_buffers_in_freepool(dev,fp);
          printf("%d\n", benchRet); 
          free(dev);
          free(fp);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          int len = 255;
          int _len_video0 = 1;
          struct uvc_video * video = (struct uvc_video *) malloc(_len_video0*sizeof(struct uvc_video));
          for(int _i0 = 0; _i0 < _len_video0; _i0++) {
            video->fid = ((-2 * (next_i()%2)) + 1) * next_i();
        video->queue.buf_used = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_buf0 = 1;
          struct uvc_buffer * buf = (struct uvc_buffer *) malloc(_len_buf0*sizeof(struct uvc_buffer));
          for(int _i0 = 0; _i0 < _len_buf0; _i0++) {
            buf->bytesused = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          ...
          int benchRet = uvc_video_encode_header(video,buf,data,len);
          printf("%d\n", benchRet); 
          free(video);
          free(buf);
          free(data);

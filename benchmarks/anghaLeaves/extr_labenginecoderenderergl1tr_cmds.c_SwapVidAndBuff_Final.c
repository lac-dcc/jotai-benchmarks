...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_config0 = 1;
          struct TYPE_3__ * config = (struct TYPE_3__ *) malloc(_len_config0*sizeof(struct TYPE_3__));
          for(int _i0 = 0; _i0 < _len_config0; _i0++) {
            config[_i0].vidWidth = ((-2 * (next_i()%2)) + 1) * next_i();
        config[_i0].buffWidth = ((-2 * (next_i()%2)) + 1) * next_i();
        config[_i0].vidHeight = ((-2 * (next_i()%2)) + 1) * next_i();
        config[_i0].buffHeight = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          SwapVidAndBuff(config);
          free(config);

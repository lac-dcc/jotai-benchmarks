...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_sp0; _i0++) {
              int _len_sp__i0__current0 = 1;
          sp[_i0].current = (int *) malloc(_len_sp__i0__current0*sizeof(int));
          for(int _j0 = 0; _j0 < _len_sp__i0__current0; _j0++) {
            sp[_i0].current[_j0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int * benchRet = r_spaces_current(sp);
          printf("%d\n", (*benchRet)); 
          for(int _aux = 0; _aux < _len_sp0; _aux++) {
          free(sp[_aux].current);
          }
          free(sp);

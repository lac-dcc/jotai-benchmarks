...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_port10; _i0++) {
            port1->iotype = ((-2 * (next_i()%2)) + 1) * next_i();
        port1->mapbase = ((-2 * (next_i()%2)) + 1) * next_i();
        port1->hub6 = ((-2 * (next_i()%2)) + 1) * next_i();
        port1->iobase = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_port20 = 1;
          struct uart_port * port2 = (struct uart_port *) malloc(_len_port20*sizeof(struct uart_port));
          for(int _i0 = 0; _i0 < _len_port20; _i0++) {
            port2->iotype = ((-2 * (next_i()%2)) + 1) * next_i();
        port2->mapbase = ((-2 * (next_i()%2)) + 1) * next_i();
        port2->hub6 = ((-2 * (next_i()%2)) + 1) * next_i();
        port2->iobase = ((-2 * (next_i()%2)) + 1) * next_i();
          }

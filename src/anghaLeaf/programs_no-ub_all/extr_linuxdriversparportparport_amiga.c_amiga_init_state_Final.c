...
__attribute__((used)) static void amiga_init_state(struct pardevice *dev, struct parport_state *s)
{
	s->u.amiga.data = 0;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_s0; _i0++) {
            s->u.amiga.datadir = ((-2 * (next_i()%2)) + 1) * next_i();
        s->u.amiga.statusdir = ((-2 * (next_i()%2)) + 1) * next_i();
        s->u.amiga.status = ((-2 * (next_i()%2)) + 1) * next_i();
        s->u.amiga.data = ((-2 * (next_i()%2)) + 1) * next_i();
          }

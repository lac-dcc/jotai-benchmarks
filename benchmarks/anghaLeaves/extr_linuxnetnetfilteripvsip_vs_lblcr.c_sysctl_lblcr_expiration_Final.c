...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_svc0 = 1;
          struct ip_vs_service * svc = (struct ip_vs_service *) malloc(_len_svc0*sizeof(struct ip_vs_service));
          for(int _i0 = 0; _i0 < _len_svc0; _i0++) {
            svc[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = sysctl_lblcr_expiration(svc);
          printf("%d\n", benchRet); 
          free(svc);

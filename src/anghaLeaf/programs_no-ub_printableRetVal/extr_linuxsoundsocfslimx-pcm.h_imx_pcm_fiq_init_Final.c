...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_params0 = 1;
          struct imx_pcm_fiq_params * params = (struct imx_pcm_fiq_params *) malloc(_len_params0*sizeof(struct imx_pcm_fiq_params));
          for(int _i0 = 0; _i0 < _len_params0; _i0++) {
            params->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = imx_pcm_fiq_init(pdev,params);
          printf("%d\n", benchRet); 
          free(pdev);
          free(params);

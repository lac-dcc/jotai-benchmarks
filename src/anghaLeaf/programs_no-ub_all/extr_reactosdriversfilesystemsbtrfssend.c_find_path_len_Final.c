...
struct TYPE_3__ *_allocate_parent(int length) {
...
void _delete_parent(struct TYPE_3__ * head) {
  if(head->parent != NULL) _delete_parent(head->parent);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    {
          int namelen = 255;
          struct TYPE_3__ * parent = _allocate_parent(1);
          int benchRet = find_path_len(parent,namelen);
          printf("%d\n", benchRet); 
          _delete_parent(parent);

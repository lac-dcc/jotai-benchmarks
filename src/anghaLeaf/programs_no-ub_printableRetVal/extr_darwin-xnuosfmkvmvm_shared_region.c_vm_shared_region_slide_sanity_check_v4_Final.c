...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_s_info0; _i0++) {
            s_info->page_size = ((-2 * (next_i()%2)) + 1) * next_i();
        s_info->page_starts_count = ((-2 * (next_i()%2)) + 1) * next_i();
        s_info->page_extras_count = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = vm_shared_region_slide_sanity_check_v4(s_info,slide_info_size);
          printf("%d\n", benchRet); 
          free(s_info);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_pResults0; _i0++) {
            pResults->testTime = ((-2 * (next_i()%2)) + 1) * next_i();
        pResults->rttAvg = ((-2 * (next_i()%2)) + 1) * next_i();
        pResults->rttMax = ((-2 * (next_i()%2)) + 1) * next_i();
        pResults->rttMin = ((-2 * (next_i()%2)) + 1) * next_i();
        pResults->numSuccsessPings = ((-2 * (next_i()%2)) + 1) * next_i();
        pResults->numSendsPings = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_pReport0 = 1;
          struct TYPE_6__ * pReport = (struct TYPE_6__ *) malloc(_len_pReport0*sizeof(struct TYPE_6__));
          for(int _i0 = 0; _i0 < _len_pReport0; _i0++) {
            pReport->TestTime = ((-2 * (next_i()%2)) + 1) * next_i();
        pReport->AvgRoundTime = ((-2 * (next_i()%2)) + 1) * next_i();
        pReport->MaxRoundTime = ((-2 * (next_i()%2)) + 1) * next_i();
        pReport->MinRoundTime = ((-2 * (next_i()%2)) + 1) * next_i();
        pReport->PacketsReceived = ((-2 * (next_i()%2)) + 1) * next_i();
        pReport->PacketsSent = ((-2 * (next_i()%2)) + 1) * next_i();
          }

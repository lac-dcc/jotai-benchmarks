...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_pipe0 = 1;
          struct isp_pipeline * pipe = (struct isp_pipeline *) malloc(_len_pipe0*sizeof(struct isp_pipeline));
          for(int _i0 = 0; _i0 < _len_pipe0; _i0++) {
            pipe->state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = isp_pipeline_ready(pipe);
          printf("%d\n", benchRet); 
          free(pipe);

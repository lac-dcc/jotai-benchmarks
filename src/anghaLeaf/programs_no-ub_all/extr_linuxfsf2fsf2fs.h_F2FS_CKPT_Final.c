...
__attribute__((used)) static inline struct f2fs_checkpoint *F2FS_CKPT(struct f2fs_sb_info *sbi)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_sbi0; _i0++) {
            sbi->ckpt = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct f2fs_checkpoint * benchRet = F2FS_CKPT(sbi);
          printf("{{struct}} %p\n", &benchRet); 
          free(sbi);

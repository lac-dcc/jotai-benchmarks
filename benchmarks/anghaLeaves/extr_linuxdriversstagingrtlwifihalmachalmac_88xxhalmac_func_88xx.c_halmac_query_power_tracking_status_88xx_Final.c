...
struct halmac_power_tracking_state_set {int process_status; } ;
struct TYPE_2__ {struct halmac_power_tracking_state_set power_tracking_set; } ;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_halmac_adapter0; _i0++) {
            halmac_adapter[_i0].halmac_state.power_tracking_set.process_status = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          ...
          enum halmac_ret_status benchRet = halmac_query_power_tracking_status_88xx(halmac_adapter,process_status,data,size);
          printf("{{other_type}} %p\n", &benchRet); 
          free(halmac_adapter);
          free(process_status);
          free(data);

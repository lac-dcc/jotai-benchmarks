...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_iob0 = 1;
          struct qeth_cmd_buffer * iob = (struct qeth_cmd_buffer *) malloc(_len_iob0*sizeof(struct qeth_cmd_buffer));
          for(int _i0 = 0; _i0 < _len_iob0; _i0++) {
            iob->data = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct qeth_ipa_cmd * benchRet = __ipa_cmd(iob);
          printf("{{struct}} %p\n", &benchRet); 
          free(iob);

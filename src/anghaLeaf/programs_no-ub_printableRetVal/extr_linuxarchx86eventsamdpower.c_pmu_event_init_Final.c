...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_event0; _i0++) {
            event->attr.config = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.type = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.sample_period = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_guest = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_host = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_idle = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_hv = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_kernel = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.exclude_user = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
__attribute__((used)) static int mt9v032_enum_mbus_code(struct v4l2_subdev *subdev,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_code0; _i0++) {
            code->index = ((-2 * (next_i()%2)) + 1) * next_i();
        code->code = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = mt9v032_enum_mbus_code(subdev,cfg,code);
          printf("%d\n", benchRet); 
          free(subdev);
          free(cfg);
          free(code);

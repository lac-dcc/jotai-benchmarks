...
__attribute__((used)) static int zoran_g_output(struct file *file, void *__fh, unsigned int *output)
{
	*output = 0;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          void * __fh;
          int _len_output0 = 1;
          unsigned int * output = (unsigned int *) malloc(_len_output0*sizeof(unsigned int));
          for(int _i0 = 0; _i0 < _len_output0; _i0++) {
            output[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = zoran_g_output(file,__fh,output);
          printf("%d\n", benchRet); 
          free(file);
          free(output);

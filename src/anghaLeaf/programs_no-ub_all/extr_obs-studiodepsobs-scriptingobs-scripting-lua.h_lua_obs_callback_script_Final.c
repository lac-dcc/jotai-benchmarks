...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_cb0 = 1;
          struct lua_obs_callback * cb = (struct lua_obs_callback *) malloc(_len_cb0*sizeof(struct lua_obs_callback));
          for(int _i0 = 0; _i0 < _len_cb0; _i0++) {
            cb->base.script = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct obs_lua_script * benchRet = lua_obs_callback_script(cb);
          printf("{{struct}} %p\n", &benchRet); 
          free(cb);

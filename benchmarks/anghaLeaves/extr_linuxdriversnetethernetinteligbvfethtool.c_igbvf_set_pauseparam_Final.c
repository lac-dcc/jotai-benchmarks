...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_pause0 = 1;
          struct ethtool_pauseparam * pause = (struct ethtool_pauseparam *) malloc(_len_pause0*sizeof(struct ethtool_pauseparam));
          for(int _i0 = 0; _i0 < _len_pause0; _i0++) {
            pause[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = igbvf_set_pauseparam(netdev,pause);
          printf("%d\n", benchRet); 
          free(netdev);
          free(pause);

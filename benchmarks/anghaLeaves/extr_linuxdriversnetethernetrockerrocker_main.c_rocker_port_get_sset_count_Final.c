...
#define  ETH_SS_STATS 128 
 int ROCKER_PORT_STATS_LEN ; 

__attribute__((used)) static int rocker_port_get_sset_count(struct net_device *netdev, int sset)
{
	switch (sset) {
	case ETH_SS_STATS:
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = rocker_port_get_sset_count(netdev,sset);
          printf("%d\n", benchRet); 
          free(netdev);

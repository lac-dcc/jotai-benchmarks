...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_config0; _i0++) {
            config->bcast_ssid_ctl = ((-2 * (next_i()%2)) + 1) * next_i();
        config->radio_ctl = ((-2 * (next_i()%2)) + 1) * next_i();
        config->dtim_period = ((-2 * (next_i()%2)) + 1) * next_i();
        config->beacon_period = ((-2 * (next_i()%2)) + 1) * next_i();
        config->auth_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        config->rts_threshold = ((-2 * (next_i()%2)) + 1) * next_i();
        config->frag_threshold = ((-2 * (next_i()%2)) + 1) * next_i();
        config->retry_limit = ((-2 * (next_i()%2)) + 1) * next_i();
        config->qos_info = ((-2 * (next_i()%2)) + 1) * next_i();
          }

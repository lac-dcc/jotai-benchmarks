...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_pdata0 = 1;
          struct max8997_platform_data * pdata = (struct max8997_platform_data *) malloc(_len_pdata0*sizeof(struct max8997_platform_data));
          for(int _i0 = 0; _i0 < _len_pdata0; _i0++) {
            pdata[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = max8997_pmic_dt_parse_pdata(pdev,pdata);
          printf("%d\n", benchRet); 
          free(pdev);
          free(pdata);

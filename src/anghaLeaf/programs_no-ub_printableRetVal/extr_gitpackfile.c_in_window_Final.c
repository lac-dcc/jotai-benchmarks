...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          long offset = 255;
          int _len_win0 = 1;
          struct pack_window * win = (struct pack_window *) malloc(_len_win0*sizeof(struct pack_window));
          for(int _i0 = 0; _i0 < _len_win0; _i0++) {
            win->offset = ((-2 * (next_i()%2)) + 1) * next_i();
        win->len = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = in_window(win,offset);
          printf("%d\n", benchRet); 
          free(win);

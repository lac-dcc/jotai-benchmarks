...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_request0 = 1;
          struct i2c_request_transaction_data * request = (struct i2c_request_transaction_data *) malloc(_len_request0*sizeof(struct i2c_request_transaction_data));
          for(int _i0 = 0; _i0 < _len_request0; _i0++) {
            request[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          dal_i2c_engine_submit_channel_request(engine,request);
          free(engine);
          free(request);

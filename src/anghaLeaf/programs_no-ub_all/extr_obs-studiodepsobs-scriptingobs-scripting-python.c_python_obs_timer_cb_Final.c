...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_timer0 = 1;
          struct python_obs_timer * timer = (struct python_obs_timer *) malloc(_len_timer0*sizeof(struct python_obs_timer));
          for(int _i0 = 0; _i0 < _len_timer0; _i0++) {
            timer->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct python_obs_callback * benchRet = python_obs_timer_cb(timer);
          printf("{{struct}} %p\n", &benchRet); 
          free(timer);

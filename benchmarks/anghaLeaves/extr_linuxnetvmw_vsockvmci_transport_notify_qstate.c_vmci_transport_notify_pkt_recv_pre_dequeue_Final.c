...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_data0 = 1;
          struct vmci_transport_recv_notify_data * data = (struct vmci_transport_recv_notify_data *) malloc(_len_data0*sizeof(struct vmci_transport_recv_notify_data));
          for(int _i0 = 0; _i0 < _len_data0; _i0++) {
            data[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = vmci_transport_notify_pkt_recv_pre_dequeue(sk,target,data);
          printf("%d\n", benchRet); 
          free(sk);
          free(data);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        instr[_i0].immediate = ((-2 * (next_i()%2)) + 1) * next_i();
        instr[_i0].src0_reg = ((-2 * (next_i()%2)) + 1) * next_i();
        instr[_i0].dest_reg = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = is_pseudo_instr_nop(instr);
          printf("%d\n", benchRet); 
          free(instr);

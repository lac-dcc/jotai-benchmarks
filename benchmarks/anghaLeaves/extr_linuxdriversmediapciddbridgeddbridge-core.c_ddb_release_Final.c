...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          file[_i0].private_data = (struct ddb *) malloc(_len_file__i0__private_data0*sizeof(struct ddb));
          for(int _j0 = 0; _j0 < _len_file__i0__private_data0; _j0++) {
            file[_i0].private_data->ddb_dev_users = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = ddb_release(inode,file);
          printf("%d\n", benchRet); 
          free(inode);

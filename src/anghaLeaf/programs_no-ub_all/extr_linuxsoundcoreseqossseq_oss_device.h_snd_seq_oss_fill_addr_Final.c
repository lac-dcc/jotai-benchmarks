...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_dp0; _i0++) {
            dp->addr = ((-2 * (next_i()%2)) + 1) * next_i();
        dp->queue = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_ev0 = 1;
          struct snd_seq_event * ev = (struct snd_seq_event *) malloc(_len_ev0*sizeof(struct snd_seq_event));
          for(int _i0 = 0; _i0 < _len_ev0; _i0++) {
            ev->dest.client = ((-2 * (next_i()%2)) + 1) * next_i();
        ev->dest.port = ((-2 * (next_i()%2)) + 1) * next_i();
        ev->source = ((-2 * (next_i()%2)) + 1) * next_i();
        ev->queue = ((-2 * (next_i()%2)) + 1) * next_i();
          }

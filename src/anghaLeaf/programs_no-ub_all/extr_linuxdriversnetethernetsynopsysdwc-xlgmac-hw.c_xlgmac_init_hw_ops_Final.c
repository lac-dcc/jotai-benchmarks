...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_hw_ops0; _i0++) {
            hw_ops->set_rss_lookup_table = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_rss_hash_key = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_rss = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_rss = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->read_mmc_stats = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->rx_mmc_int = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->tx_mmc_int = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_pblx8 = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->get_tx_pbl_val = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_tx_pbl_val = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->get_rx_pbl_val = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rx_pbl_val = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_osp_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_tsf_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rsf_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_tx_threshold = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rx_threshold = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->riwt_to_usec = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->usec_to_riwt = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_tx_coalesce = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rx_coalesce = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->update_vlan_hash_table = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_rx_vlan_filtering = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_rx_vlan_filtering = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_rx_vlan_stripping = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_rx_vlan_stripping = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rx_flow_control = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_tx_flow_control = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->tx_start_xmit = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->is_context_desc = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->is_last_desc = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->rx_desc_reset = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->tx_desc_reset = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->rx_desc_init = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->tx_desc_init = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_xlgmii_100000_speed = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_xlgmii_50000_speed = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_xlgmii_40000_speed = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_xlgmii_25000_speed = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_rx_csum = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_rx_csum = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->config_rx_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->set_mac_address = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_int = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_int = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->dev_read = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->dev_xmit = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_rx = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_rx = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->disable_tx = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->enable_tx = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->tx_complete = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->exit = ((-2 * (next_i()%2)) + 1) * next_i();
        hw_ops->init = ((-2 * (next_i()%2)) + 1) * next_i();
          }

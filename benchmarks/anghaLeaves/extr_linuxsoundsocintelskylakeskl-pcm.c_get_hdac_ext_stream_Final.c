...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          substream[_i0].runtime->private_data = (struct hdac_ext_stream *) malloc(_len_substream__i0__runtime_private_data0*sizeof(struct hdac_ext_stream));
          for(int _j0 = 0; _j0 < _len_substream__i0__runtime_private_data0; _j0++) {
            substream[_i0].runtime->private_data->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          struct hdac_ext_stream * benchRet = get_hdac_ext_stream(substream);
          printf("%d\n", (*benchRet).dummy);

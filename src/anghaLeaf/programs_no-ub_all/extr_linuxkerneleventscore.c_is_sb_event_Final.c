...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_event0; _i0++) {
            event->attach_state = ((-2 * (next_i()%2)) + 1) * next_i();
        event->parent = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.context_switch = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.task = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.comm_exec = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.comm = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.mmap2 = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.mmap_data = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.mmap = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_rc0; _i0++) {
            rc->bottom = ((-2 * (next_i()%2)) + 1) * next_i();
        rc->right = ((-2 * (next_i()%2)) + 1) * next_i();
        rc->top = ((-2 * (next_i()%2)) + 1) * next_i();
        rc->left = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int * benchRet = element_from_margin_id(rc,id);
          printf("{{other_type}} %p\n", &benchRet); 
          free(rc);

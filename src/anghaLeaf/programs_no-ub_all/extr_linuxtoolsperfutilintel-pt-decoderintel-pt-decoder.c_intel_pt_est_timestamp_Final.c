...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_decoder0; _i0++) {
            decoder->sample_insn_cnt = ((-2 * (next_i()%2)) + 1) * next_i();
        decoder->cbr = ((-2 * (next_i()%2)) + 1) * next_i();
        decoder->max_non_turbo_ratio = ((-2 * (next_i()%2)) + 1) * next_i();
        decoder->sample_timestamp = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = intel_pt_est_timestamp(decoder);
          printf("%d\n", benchRet); 
          free(decoder);

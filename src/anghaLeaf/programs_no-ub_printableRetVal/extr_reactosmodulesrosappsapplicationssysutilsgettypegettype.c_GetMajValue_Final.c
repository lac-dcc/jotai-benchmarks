...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_osvi0; _i0++) {
            osvi->dwMajorVersion = ((-2 * (next_i()%2)) + 1) * next_i();
        osvi->dwMinorVersion = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_pBuf1020 = 1;
          struct TYPE_5__ * pBuf102 = (struct TYPE_5__ *) malloc(_len_pBuf1020*sizeof(struct TYPE_5__));
          for(int _i0 = 0; _i0 < _len_pBuf1020; _i0++) {
            pBuf102->sv102_version_major = ((-2 * (next_i()%2)) + 1) * next_i();
        pBuf102->sv102_version_minor = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = GetMajValue(Major,bLocal,osvi,pBuf102);
          printf("%d\n", benchRet); 
          free(osvi);
          free(pBuf102);

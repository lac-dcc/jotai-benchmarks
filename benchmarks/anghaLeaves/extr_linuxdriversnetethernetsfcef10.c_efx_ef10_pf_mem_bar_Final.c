...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_efx0; _i0++) {
              int _len_efx__i0__pci_dev0 = 1;
          efx[_i0].pci_dev = (struct TYPE_2__ *) malloc(_len_efx__i0__pci_dev0*sizeof(struct TYPE_2__));
          for(int _j0 = 0; _j0 < _len_efx__i0__pci_dev0; _j0++) {
            efx[_i0].pci_dev->device = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          unsigned int benchRet = efx_ef10_pf_mem_bar(efx);
          printf("%u\n", benchRet); 
          for(int _aux = 0; _aux < _len_efx0; _aux++) {
          free(efx[_aux].pci_dev);
          }
          free(efx);

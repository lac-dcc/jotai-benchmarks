...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_source0; _i0++) {
            source->push_to_mute_stop_time = ((-2 * (next_i()%2)) + 1) * next_i();
        source->push_to_mute_delay = ((-2 * (next_i()%2)) + 1) * next_i();
        source->push_to_talk_stop_time = ((-2 * (next_i()%2)) + 1) * next_i();
        source->push_to_talk_delay = ((-2 * (next_i()%2)) + 1) * next_i();
        source->push_to_talk_enabled = ((-2 * (next_i()%2)) + 1) * next_i();
        source->push_to_mute_enabled = ((-2 * (next_i()%2)) + 1) * next_i();
        source->user_muted = ((-2 * (next_i()%2)) + 1) * next_i();
        source->enabled = ((-2 * (next_i()%2)) + 1) * next_i();
        source->user_push_to_talk_pressed = ((-2 * (next_i()%2)) + 1) * next_i();
        source->user_push_to_mute_pressed = ((-2 * (next_i()%2)) + 1) * next_i();
          }

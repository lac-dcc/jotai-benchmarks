...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          fe[_i0].tuner_priv = (struct stb6100_state *) malloc(_len_fe__i0__tuner_priv0*sizeof(struct stb6100_state));
          for(int _j0 = 0; _j0 < _len_fe__i0__tuner_priv0; _j0++) {
            fe[_i0].tuner_priv->bandwidth = ((-2 * (next_i()%2)) + 1) * next_i();
        fe[_i0].tuner_priv->reference = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = stb6100_init(fe);
          printf("%d\n", benchRet); 

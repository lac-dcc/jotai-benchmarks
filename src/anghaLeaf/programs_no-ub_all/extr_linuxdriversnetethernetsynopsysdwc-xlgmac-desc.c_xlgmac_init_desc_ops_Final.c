...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_desc_ops0; _i0++) {
            desc_ops->rx_desc_init = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->tx_desc_init = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->unmap_desc_data = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->map_rx_buffer = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->map_tx_skb = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->free_channels_and_rings = ((-2 * (next_i()%2)) + 1) * next_i();
        desc_ops->alloc_channles_and_rings = ((-2 * (next_i()%2)) + 1) * next_i();
          }

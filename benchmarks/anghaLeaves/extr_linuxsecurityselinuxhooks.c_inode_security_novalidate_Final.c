...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          inode[_i0].i_security = (struct inode_security_struct *) malloc(_len_inode__i0__i_security0*sizeof(struct inode_security_struct));
          for(int _j0 = 0; _j0 < _len_inode__i0__i_security0; _j0++) {
            inode[_i0].i_security->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct inode_security_struct * benchRet = inode_security_novalidate(inode);
          printf("%d\n", (*benchRet).dummy);

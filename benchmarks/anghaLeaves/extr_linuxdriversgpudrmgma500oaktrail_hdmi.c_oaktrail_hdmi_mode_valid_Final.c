...
__attribute__((used)) static enum drm_mode_status oaktrail_hdmi_mode_valid(struct drm_connector *connector,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        mode[_i0].flags = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          enum drm_mode_status benchRet = oaktrail_hdmi_mode_valid(connector,mode);
          free(connector);
          free(mode);

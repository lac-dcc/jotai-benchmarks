...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->rt_priority = ((-2 * (next_i()%2)) + 1) * next_i();
        p->dl.flags = ((-2 * (next_i()%2)) + 1) * next_i();
        p->dl.dl_period = ((-2 * (next_i()%2)) + 1) * next_i();
        p->dl.dl_deadline = ((-2 * (next_i()%2)) + 1) * next_i();
        p->dl.dl_runtime = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_attr0 = 1;
          struct sched_attr * attr = (struct sched_attr *) malloc(_len_attr0*sizeof(struct sched_attr));
          for(int _i0 = 0; _i0 < _len_attr0; _i0++) {
            attr->sched_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        attr->sched_period = ((-2 * (next_i()%2)) + 1) * next_i();
        attr->sched_deadline = ((-2 * (next_i()%2)) + 1) * next_i();
        attr->sched_runtime = ((-2 * (next_i()%2)) + 1) * next_i();
        attr->sched_priority = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
/* Type definitions */
struct amdgpu_queue_mgr {int dummy; } ;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_mgr0 = 1;
          struct amdgpu_queue_mgr * mgr = (struct amdgpu_queue_mgr *) malloc(_len_mgr0*sizeof(struct amdgpu_queue_mgr));
          for(int _i0 = 0; _i0 < _len_mgr0; _i0++) {
            mgr[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = amdgpu_queue_mgr_fini(adev,mgr);
          printf("%d\n", benchRet); 
          free(adev);
          free(mgr);

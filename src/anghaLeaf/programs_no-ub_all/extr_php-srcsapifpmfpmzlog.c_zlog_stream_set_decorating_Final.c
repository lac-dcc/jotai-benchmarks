...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_stream0; _i0++) {
            stream->decorate = ((-2 * (next_i()%2)) + 1) * next_i();
        stream->prefix_buffer = ((-2 * (next_i()%2)) + 1) * next_i();
        stream->msg_quote = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          zlog_stream_set_decorating(stream,decorate);
          free(stream);

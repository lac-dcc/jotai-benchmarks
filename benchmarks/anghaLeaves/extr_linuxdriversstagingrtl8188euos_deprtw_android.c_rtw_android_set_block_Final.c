...
__attribute__((used)) static int rtw_android_set_block(struct net_device *net, char *command,
				 int total_len)
{
	return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_command0 = 1;
          char * command = (char *) malloc(_len_command0*sizeof(char));
          for(int _i0 = 0; _i0 < _len_command0; _i0++) {
            command[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = rtw_android_set_block(net,command,total_len);
          printf("%d\n", benchRet); 
          free(net);
          free(command);

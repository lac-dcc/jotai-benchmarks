...
struct jffs2_raw_node_ref *_allocate_raw(int length) {
  struct jffs2_raw_node_ref *head = (struct jffs2_raw_node_ref *)malloc(sizeof(struct jffs2_raw_node_ref));

  head->next_in_ino = NULL;

  struct jffs2_raw_node_ref *walker = head;
...
void _delete_raw(struct jffs2_raw_node_ref * head) {
  if(head->next_in_ino != NULL) _delete_raw(head->next_in_ino);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct jffs2_raw_node_ref * raw = _allocate_raw(1);

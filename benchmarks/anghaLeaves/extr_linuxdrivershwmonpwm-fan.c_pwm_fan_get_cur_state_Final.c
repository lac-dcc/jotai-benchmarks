...
__attribute__((used)) static int pwm_fan_get_cur_state(struct thermal_cooling_device *cdev,
				 unsigned long *state)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_cdev__i0__devdata0; _j0++) {
            cdev[_i0].devdata->pwm_fan_state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_state0 = 1;
          ...
          int benchRet = pwm_fan_get_cur_state(cdev,state);
          printf("%d\n", benchRet); 

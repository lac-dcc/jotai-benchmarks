...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_fl10; _i0++) {
            fl1->daddr = ((-2 * (next_i()%2)) + 1) * next_i();
        fl1->saddr = ((-2 * (next_i()%2)) + 1) * next_i();
        fl1->flowidn_mark = ((-2 * (next_i()%2)) + 1) * next_i();
        fl1->flowidn_scope = ((-2 * (next_i()%2)) + 1) * next_i();
        fl1->flowidn_oif = ((-2 * (next_i()%2)) + 1) * next_i();
        fl1->flowidn_iif = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_fl20 = 1;
          struct flowidn * fl2 = (struct flowidn *) malloc(_len_fl20*sizeof(struct flowidn));
          for(int _i0 = 0; _i0 < _len_fl20; _i0++) {
            fl2->daddr = ((-2 * (next_i()%2)) + 1) * next_i();
        fl2->saddr = ((-2 * (next_i()%2)) + 1) * next_i();
        fl2->flowidn_mark = ((-2 * (next_i()%2)) + 1) * next_i();
        fl2->flowidn_scope = ((-2 * (next_i()%2)) + 1) * next_i();
        fl2->flowidn_oif = ((-2 * (next_i()%2)) + 1) * next_i();
        fl2->flowidn_iif = ((-2 * (next_i()%2)) + 1) * next_i();
          }

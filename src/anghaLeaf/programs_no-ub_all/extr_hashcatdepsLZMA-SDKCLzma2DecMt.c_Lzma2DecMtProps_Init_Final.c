...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->inBufSize_ST = ((-2 * (next_i()%2)) + 1) * next_i();
        p->outStep_ST = ((-2 * (next_i()%2)) + 1) * next_i();
        p->numThreads = ((-2 * (next_i()%2)) + 1) * next_i();
        p->inBufSize_MT = ((-2 * (next_i()%2)) + 1) * next_i();
        p->outBlockMax = ((-2 * (next_i()%2)) + 1) * next_i();
        p->inBlockMax = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          Lzma2DecMtProps_Init(p);
          free(p);

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_port0 = 1;
          struct tegra_pcie_port * port = (struct tegra_pcie_port *) malloc(_len_port0*sizeof(struct tegra_pcie_port));
          for(int _i0 = 0; _i0 < _len_port0; _i0++) {
            port[_i0].index = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned long benchRet = tegra_pcie_port_get_pex_ctrl(port);
          printf("%lu\n", benchRet); 
          free(port);

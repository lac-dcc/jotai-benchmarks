...
__attribute__((used)) static inline struct f2fs_sb_info *F2FS_SB(struct super_block *sb)
{
	return sb->s_fs_info;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          sb[_i0].s_fs_info = (struct f2fs_sb_info *) malloc(_len_sb__i0__s_fs_info0*sizeof(struct f2fs_sb_info));
          for(int _j0 = 0; _j0 < _len_sb__i0__s_fs_info0; _j0++) {
            sb[_i0].s_fs_info->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct f2fs_sb_info * benchRet = F2FS_SB(sb);
          printf("%d\n", (*benchRet).dummy);

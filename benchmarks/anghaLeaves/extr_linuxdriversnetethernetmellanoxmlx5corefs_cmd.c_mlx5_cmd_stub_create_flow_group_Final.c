...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_group_id0 = 1;
          unsigned int * group_id = (unsigned int *) malloc(_len_group_id0*sizeof(unsigned int));
          for(int _i0 = 0; _i0 < _len_group_id0; _i0++) {
            group_id[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = mlx5_cmd_stub_create_flow_group(dev,ft,in,group_id);
          printf("%d\n", benchRet); 
          free(dev);
          free(ft);
          free(in);

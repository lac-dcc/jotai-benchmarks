...
__attribute__((used)) static int snd_sonicvibes_info_double(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_kcontrol0; _i0++) {
            kcontrol->private_value = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_uinfo0 = 1;
        ...
        uinfo->type = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = snd_sonicvibes_info_double(kcontrol,uinfo);
          printf("%d\n", benchRet); 
          free(kcontrol);
          free(uinfo);

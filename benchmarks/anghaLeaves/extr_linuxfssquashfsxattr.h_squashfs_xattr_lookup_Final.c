...
__attribute__((used)) static inline int squashfs_xattr_lookup(struct super_block *sb,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_xattr0 = 1;
          unsigned long long * xattr = (unsigned long long *) malloc(_len_xattr0*sizeof(unsigned long long));
          for(int _i0 = 0; _i0 < _len_xattr0; _i0++) {
            xattr[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = squashfs_xattr_lookup(sb,index,count,size,xattr);
          printf("%d\n", benchRet); 
          free(sb);
          free(count);
          free(size);

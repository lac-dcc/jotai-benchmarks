...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_item0 = 1;
          struct tree * item = (struct tree *) malloc(_len_item0*sizeof(struct tree));
          for(int _i0 = 0; _i0 < _len_item0; _i0++) {
              item[_i0].size = ((-2 * (next_i()%2)) + 1) * next_i();
          item[_i0].object.parsed = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = parse_tree_buffer(item,buffer,size);
          printf("%d\n", benchRet); 
          free(item);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_tif0; _i0++) {
            tif->tif_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_deftilesize = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_defstripsize = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_seek = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_encodetile = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_encodestrip = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_encoderow = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_decodetile = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_decodestrip = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_decoderow = ((-2 * (next_i()%2)) + 1) * next_i();
        tif->tif_fixuptags = ((-2 * (next_i()%2)) + 1) * next_i();
          }

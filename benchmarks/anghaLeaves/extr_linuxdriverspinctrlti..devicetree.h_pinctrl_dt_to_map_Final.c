...
__attribute__((used)) static inline int pinctrl_dt_to_map(struct pinctrl *p,
...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_p0 = 1;
          struct pinctrl * p = (struct pinctrl *) malloc(_len_p0*sizeof(struct pinctrl));
          ...
          int benchRet = pinctrl_dt_to_map(p,pctldev);
          printf("%d\n", benchRet); 
          free(p);
          free(pctldev);

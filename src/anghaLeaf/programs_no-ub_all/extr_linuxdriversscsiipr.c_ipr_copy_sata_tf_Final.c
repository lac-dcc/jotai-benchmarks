...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_regs0; _i0++) {
            regs->ctl = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->hob_lbah = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->hob_lbam = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->hob_lbal = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->hob_nsect = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->hob_feature = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->command = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->device = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->lbah = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->lbam = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->lbal = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->nsect = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->feature = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_tf0 = 1;
          struct ata_taskfile * tf = (struct ata_taskfile *) malloc(_len_tf0*sizeof(struct ata_taskfile));
          for(int _i0 = 0; _i0 < _len_tf0; _i0++) {
            tf->ctl = ((-2 * (next_i()%2)) + 1) * next_i();

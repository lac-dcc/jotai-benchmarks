...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_hdw0; _i0++) {
            hdw[_i0].unit_number = ((-2 * (next_i()%2)) + 1) * next_i();
        hdw[_i0].tuner_type = ((-2 * (next_i()%2)) + 1) * next_i();
        hdw[_i0].tuner_updated = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = get_default_tuner_type(hdw);
          printf("%d\n", benchRet); 
          free(hdw);

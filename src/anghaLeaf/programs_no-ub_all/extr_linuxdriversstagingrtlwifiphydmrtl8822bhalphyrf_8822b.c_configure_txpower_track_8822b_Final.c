...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_config0; _i0++) {
            config->get_delta_swing_table = ((-2 * (next_i()%2)) + 1) * next_i();
        config->phy_lc_calibrate = ((-2 * (next_i()%2)) + 1) * next_i();
        config->do_iqk = ((-2 * (next_i()%2)) + 1) * next_i();
        config->odm_tx_pwr_track_set_pwr = ((-2 * (next_i()%2)) + 1) * next_i();
        config->thermal_reg_addr = ((-2 * (next_i()%2)) + 1) * next_i();
        config->rf_path_count = ((-2 * (next_i()%2)) + 1) * next_i();
        config->average_thermal_num = ((-2 * (next_i()%2)) + 1) * next_i();
        config->threshold_dpk = ((-2 * (next_i()%2)) + 1) * next_i();
        config->threshold_iqk = ((-2 * (next_i()%2)) + 1) * next_i();
          }

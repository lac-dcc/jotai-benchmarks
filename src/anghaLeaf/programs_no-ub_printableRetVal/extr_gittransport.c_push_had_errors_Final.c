...
struct ref *_allocate_ref(int length) {
  struct ref *head = (struct ref *)malloc(sizeof(struct ref));

  head->status = rand();
  head->next = NULL;

  struct ref *walker = head;
...
void _delete_ref(struct ref * head) {
  if(head->next != NULL) _delete_ref(head->next);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct ref * ref = _allocate_ref(1);

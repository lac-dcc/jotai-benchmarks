...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_card0; _i0++) {
            card->curr_wr_port = ((-2 * (next_i()%2)) + 1) * next_i();
        card->mp_end_port = ((-2 * (next_i()%2)) + 1) * next_i();
        card->mp_agg_pkt_limit = ((-2 * (next_i()%2)) + 1) * next_i();
        card->max_ports = ((-2 * (next_i()%2)) + 1) * next_i();
        card->mpa_tx.start_port = ((-2 * (next_i()%2)) + 1) * next_i();
        card->supports_sdio_new_mode = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = mp_tx_aggr_port_limit_reached(card);
          printf("%d\n", benchRet); 
          free(card);

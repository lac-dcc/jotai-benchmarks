...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_usbvision0; _i0++) {
            usbvision->last_isoc_frame_num = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->compr_level = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->last_compr_level = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->request_intra = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->isoc_measure_bandwidth_count = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->isoc_urb_count = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->isoc_skip_count = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->isoc_packet_count = ((-2 * (next_i()%2)) + 1) * next_i();
        usbvision->isoc_data_count = ((-2 * (next_i()%2)) + 1) * next_i();
          }

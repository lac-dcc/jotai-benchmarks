...
struct curl_slist *_allocate_list(int length, struct curl_slist *aux_list[]) {
  struct curl_slist *walker = (struct curl_slist *)malloc(sizeof(struct curl_slist));

  aux_list[0] = walker;
  walker->next = NULL;

  struct curl_slist *head = walker;
...
void _delete_list(struct curl_slist *aux_list[], int aux_list_size) {

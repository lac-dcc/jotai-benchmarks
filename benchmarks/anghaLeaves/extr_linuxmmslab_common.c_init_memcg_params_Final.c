...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_root_cache0 = 1;
          struct kmem_cache * root_cache = (struct kmem_cache *) malloc(_len_root_cache0*sizeof(struct kmem_cache));
          for(int _i0 = 0; _i0 < _len_root_cache0; _i0++) {
            root_cache[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = init_memcg_params(s,memcg,root_cache);
          printf("%d\n", benchRet); 
          free(s);
          free(memcg);
          free(root_cache);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_priv0; _i0++) {
            priv->tx_clean_pointer = ((-2 * (next_i()%2)) + 1) * next_i();
        priv->tx_pointer = ((-2 * (next_i()%2)) + 1) * next_i();
        priv->tx_q_entries = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ftgmac100_tx_buf_avail(priv);
          printf("%d\n", benchRet); 
          free(priv);

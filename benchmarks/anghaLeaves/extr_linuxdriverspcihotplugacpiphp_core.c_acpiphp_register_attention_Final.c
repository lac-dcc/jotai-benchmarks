...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info[_i0].get_attn = ((-2 * (next_i()%2)) + 1) * next_i();
        info[_i0].set_attn = ((-2 * (next_i()%2)) + 1) * next_i();
        info[_i0].owner = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = acpiphp_register_attention(info);
          printf("%d\n", benchRet); 
          free(info);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_root0; _i0++) {
            root->reinit = ((-2 * (next_i()%2)) + 1) * next_i();
        root->last_size = ((-2 * (next_i()%2)) + 1) * next_i();
        root->size = ((-2 * (next_i()%2)) + 1) * next_i();
        root->prev_size = ((-2 * (next_i()%2)) + 1) * next_i();
        root->last_mscr_area = ((-2 * (next_i()%2)) + 1) * next_i();
        root->mscr_area = ((-2 * (next_i()%2)) + 1) * next_i();
        root->prev_chi = ((-2 * (next_i()%2)) + 1) * next_i();
        root->chi = ((-2 * (next_i()%2)) + 1) * next_i();
        root->step_now = ((-2 * (next_i()%2)) + 1) * next_i();
        root->rank = ((-2 * (next_i()%2)) + 1) * next_i();
        root->max_point = ((-2 * (next_i()%2)) + 1) * next_i();
        root->min_point = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_node0 = 1;
          struct TYPE_5__ * node = (struct TYPE_5__ *) malloc(_len_node0*sizeof(struct TYPE_5__));
          for(int _i0 = 0; _i0 < _len_node0; _i0++) {
            node->point = ((-2 * (next_i()%2)) + 1) * next_i();
          }

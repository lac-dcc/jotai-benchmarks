...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_envelope0; _i0++) {
            envelope->attack_level = ((-2 * (next_i()%2)) + 1) * next_i();
        envelope->fade_level = ((-2 * (next_i()%2)) + 1) * next_i();
        envelope->attack_length = ((-2 * (next_i()%2)) + 1) * next_i();
        envelope->fade_length = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_old0 = 1;
          struct ff_envelope * old = (struct ff_envelope *) malloc(_len_old0*sizeof(struct ff_envelope));
          for(int _i0 = 0; _i0 < _len_old0; _i0++) {
            old->attack_level = ((-2 * (next_i()%2)) + 1) * next_i();
        old->fade_level = ((-2 * (next_i()%2)) + 1) * next_i();
        old->attack_length = ((-2 * (next_i()%2)) + 1) * next_i();
        old->fade_length = ((-2 * (next_i()%2)) + 1) * next_i();
          }

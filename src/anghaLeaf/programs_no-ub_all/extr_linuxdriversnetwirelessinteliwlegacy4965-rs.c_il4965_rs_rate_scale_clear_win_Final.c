...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_win0 = 1;
          struct il_rate_scale_data * win = (struct il_rate_scale_data *) malloc(_len_win0*sizeof(struct il_rate_scale_data));
          for(int _i0 = 0; _i0 < _len_win0; _i0++) {
            win->stamp = ((-2 * (next_i()%2)) + 1) * next_i();
        win->counter = ((-2 * (next_i()%2)) + 1) * next_i();
        win->success_counter = ((-2 * (next_i()%2)) + 1) * next_i();
        win->data = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          il4965_rs_rate_scale_clear_win(win);
          free(win);

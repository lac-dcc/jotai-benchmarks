...
__attribute__((used)) static int az6027_rc_query(struct dvb_usb_device *d, u32 *event, int *state)
{
	return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_event0 = 1;
          int * event = (int *) malloc(_len_event0*sizeof(int));
          for(int _i0 = 0; _i0 < _len_event0; _i0++) {
            event[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_state0 = 1;
          ...
          int benchRet = az6027_rc_query(d,event,state);
          printf("%d\n", benchRet); 
          free(d);
          free(event);
          free(state);

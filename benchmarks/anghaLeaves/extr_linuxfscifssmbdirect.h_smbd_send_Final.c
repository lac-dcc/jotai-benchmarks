...
__attribute__((used)) static inline int smbd_send(struct TCP_Server_Info *server, struct smb_rqst *rqst) {return -1; }
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_rqst0; _i0++) {
            rqst[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = smbd_send(server,rqst);
          printf("%d\n", benchRet); 
          free(server);
          free(rqst);

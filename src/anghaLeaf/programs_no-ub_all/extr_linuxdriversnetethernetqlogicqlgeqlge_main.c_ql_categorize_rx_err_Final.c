...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_qdev0; _i0++) {
            qdev->nic_stats.rx_crc_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_frame_len_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_preamble_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_undersize_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_oversize_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_code_err = ((-2 * (next_i()%2)) + 1) * next_i();
        qdev->nic_stats.rx_err_count = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_rx_ring0 = 1;
          struct rx_ring * rx_ring = (struct rx_ring *) malloc(_len_rx_ring0*sizeof(struct rx_ring));
          for(int _i0 = 0; _i0 < _len_rx_ring0; _i0++) {
            rx_ring->rx_errors = ((-2 * (next_i()%2)) + 1) * next_i();
          }

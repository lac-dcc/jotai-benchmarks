...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_state0; _i0++) {
            state->level = ((-2 * (next_i()%2)) + 1) * next_i();
        state->idx2 = ((-2 * (next_i()%2)) + 1) * next_i();
        state->idx1 = ((-2 * (next_i()%2)) + 1) * next_i();
        state->service_counter = ((-2 * (next_i()%2)) + 1) * next_i();
        state->request_counter = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          state_init(state);
          free(state);

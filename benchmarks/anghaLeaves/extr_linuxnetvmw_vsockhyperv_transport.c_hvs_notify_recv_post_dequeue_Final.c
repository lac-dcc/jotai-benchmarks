...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_d0 = 1;
          struct vsock_transport_recv_notify_data * d = (struct vsock_transport_recv_notify_data *) malloc(_len_d0*sizeof(struct vsock_transport_recv_notify_data));
          for(int _i0 = 0; _i0 < _len_d0; _i0++) {
            d[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = hvs_notify_recv_post_dequeue(vsk,target,copied,data_read,d);
          printf("%d\n", benchRet); 
          free(vsk);
          free(d);

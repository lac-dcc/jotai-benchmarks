...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_ctx0 = 1;
          struct context * ctx = (struct context *) malloc(_len_ctx0*sizeof(struct context));
          ...
          int _len_server0 = 1;
          struct server * server = (struct server *) malloc(_len_server0*sizeof(struct server));
          for(int _i0 = 0; _i0 < _len_server0; _i0++) {
            server[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          memcache_post_connect(ctx,conn,server);
          free(ctx);
          free(conn);
          free(server);

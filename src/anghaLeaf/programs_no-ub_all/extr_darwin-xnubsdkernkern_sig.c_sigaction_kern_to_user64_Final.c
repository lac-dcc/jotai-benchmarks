...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_in0; _i0++) {
            in->sa_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        in->sa_mask = ((-2 * (next_i()%2)) + 1) * next_i();
        in->__sigaction_u.__sa_handler = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_out0 = 1;
          struct user64_sigaction * out = (struct user64_sigaction *) malloc(_len_out0*sizeof(struct user64_sigaction));
          for(int _i0 = 0; _i0 < _len_out0; _i0++) {
            out->sa_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        out->sa_mask = ((-2 * (next_i()%2)) + 1) * next_i();
        out->__sigaction_u.__sa_handler = ((-2 * (next_i()%2)) + 1) * next_i();
          }

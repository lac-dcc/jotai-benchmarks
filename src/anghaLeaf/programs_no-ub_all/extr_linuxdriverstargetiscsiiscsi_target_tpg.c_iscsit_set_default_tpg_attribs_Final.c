...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_tpg0; _i0++) {
            tpg->tpg_attrib.login_keys_workaround = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.tpg_enabled_sendtargets = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.fabric_prot_type = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.t10_pi = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.default_erl = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.demo_mode_discovery = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.prod_mode_write_protect = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.demo_mode_write_protect = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.cache_dynamic_acls = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.generate_node_acls = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.default_cmdsn_depth = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.netif_timeout = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.login_timeout = ((-2 * (next_i()%2)) + 1) * next_i();
        tpg->tpg_attrib.authentication = ((-2 * (next_i()%2)) + 1) * next_i();
          }

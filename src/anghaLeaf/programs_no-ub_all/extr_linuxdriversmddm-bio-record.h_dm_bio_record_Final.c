...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_bd0; _i0++) {
            bd->bi_iter = ((-2 * (next_i()%2)) + 1) * next_i();
        bd->bi_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        bd->bi_partno = ((-2 * (next_i()%2)) + 1) * next_i();
        bd->bi_disk = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_bio0 = 1;
          struct bio * bio = (struct bio *) malloc(_len_bio0*sizeof(struct bio));
          for(int _i0 = 0; _i0 < _len_bio0; _i0++) {
            bio->bi_iter = ((-2 * (next_i()%2)) + 1) * next_i();
        bio->bi_flags = ((-2 * (next_i()%2)) + 1) * next_i();
        bio->bi_partno = ((-2 * (next_i()%2)) + 1) * next_i();
        bio->bi_disk = ((-2 * (next_i()%2)) + 1) * next_i();
          }

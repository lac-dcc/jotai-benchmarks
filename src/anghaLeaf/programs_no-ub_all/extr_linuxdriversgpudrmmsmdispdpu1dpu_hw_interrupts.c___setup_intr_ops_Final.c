...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ops0; _i0++) {
            ops->get_interrupt_status = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->clear_intr_status_nolock = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->clear_interrupt_status = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->get_interrupt_statuses = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->get_valid_interrupts = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->disable_all_irqs = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->clear_all_irqs = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->dispatch_irqs = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->disable_irq = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->enable_irq = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->irq_idx_lookup = ((-2 * (next_i()%2)) + 1) * next_i();
        ops->set_mask = ((-2 * (next_i()%2)) + 1) * next_i();
          }

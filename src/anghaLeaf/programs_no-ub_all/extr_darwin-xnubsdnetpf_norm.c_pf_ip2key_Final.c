...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_key0; _i0++) {
            key->fr_dstx.v4addr.s_addr = ((-2 * (next_i()%2)) + 1) * next_i();
        key->fr_srcx.v4addr.s_addr = ((-2 * (next_i()%2)) + 1) * next_i();
        key->fr_af = ((-2 * (next_i()%2)) + 1) * next_i();
        key->fr_id = ((-2 * (next_i()%2)) + 1) * next_i();
        key->fr_p = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_ip0 = 1;
          struct ip * ip = (struct ip *) malloc(_len_ip0*sizeof(struct ip));
          for(int _i0 = 0; _i0 < _len_ip0; _i0++) {
            ip->ip_dst.s_addr = ((-2 * (next_i()%2)) + 1) * next_i();
        ip->ip_src.s_addr = ((-2 * (next_i()%2)) + 1) * next_i();
        ip->ip_id = ((-2 * (next_i()%2)) + 1) * next_i();
        ip->ip_p = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          pf_ip2key(key,ip);
          free(key);
          free(ip);

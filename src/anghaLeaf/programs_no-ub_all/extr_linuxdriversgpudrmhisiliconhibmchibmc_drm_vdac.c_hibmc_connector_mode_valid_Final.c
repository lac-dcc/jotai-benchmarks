...
__attribute__((used)) static enum drm_mode_status hibmc_connector_mode_valid(struct drm_connector *connector,
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_mode0; _i0++) {
            mode->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          enum drm_mode_status benchRet = hibmc_connector_mode_valid(connector,mode);
          printf("{{other_type}} %p\n", &benchRet); 
          free(connector);
          free(mode);

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_p0 = 1;
          struct aux_payloads * p = (struct aux_payloads *) malloc(_len_p0*sizeof(struct aux_payloads));
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->payloads.container = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct aux_payload * benchRet = dal_ddc_aux_payloads_get(p);
          printf("{{struct}} %p\n", &benchRet); 
          free(p);

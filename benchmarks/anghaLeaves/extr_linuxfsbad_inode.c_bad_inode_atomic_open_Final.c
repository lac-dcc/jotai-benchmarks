...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int benchRet = bad_inode_atomic_open(inode,dentry,file,open_flag,create_mode);
          printf("%d\n", benchRet); 
          free(inode);
          free(dentry);
          free(file);
    ...
    case 3:
    {
          unsigned int open_flag = ((-2 * (next_i()%2)) + 1) * next_i();
        
          int create_mode = ((-2 * (next_i()%2)) + 1) * next_i();

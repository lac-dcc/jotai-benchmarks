...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        c[_i0].Out.unprocessed_bytes = ((-2 * (next_i()%2)) + 1) * next_i();
        c[_i0].out.total_bytes = ((-2 * (next_i()%2)) + 1) * next_i();
        c[_i0].crypto = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = out_total_unprocessed_bytes(c);
          printf("%d\n", benchRet); 
          free(c);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_r__i0__connection0; _j0++) {
            r[_i0].connection->proxy_protocol_addr.data = ((-2 * (next_i()%2)) + 1) * next_i();
        r[_i0].connection->proxy_protocol_addr.len = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_v0 = 1;
        ...
        v[_i0].len = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = ngx_http_variable_proxy_protocol_addr(r,v,data);
          printf("%d\n", benchRet); 

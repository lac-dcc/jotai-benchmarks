...
struct TYPE_4__ *_allocate_p_node(int length) {
...
void _delete_p_node(struct TYPE_4__ * head) {
  if(head->next != NULL) _delete_p_node(head->next);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_4__ * p_node = _allocate_p_node(1);
          struct TYPE_4__ * benchRet = _cdio_list_node_next(p_node);
          printf("{{struct}} %p\n", &benchRet); 
          _delete_p_node(p_node);

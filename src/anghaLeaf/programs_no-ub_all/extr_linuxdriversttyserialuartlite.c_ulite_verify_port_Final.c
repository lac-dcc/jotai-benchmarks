...
__attribute__((used)) static int ulite_verify_port(struct uart_port *port, struct serial_struct *ser)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ser0; _i0++) {
            ser->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = ulite_verify_port(port,ser);
          printf("%d\n", benchRet); 
          free(port);
          free(ser);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_effect0; _i0++) {
            effect->direction = ((-2 * (next_i()%2)) + 1) * next_i();
        effect->replay.length = ((-2 * (next_i()%2)) + 1) * next_i();
        effect->replay.delay = ((-2 * (next_i()%2)) + 1) * next_i();
        effect->trigger.interval = ((-2 * (next_i()%2)) + 1) * next_i();
        effect->trigger.button = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_old0 = 1;
          struct ff_effect * old = (struct ff_effect *) malloc(_len_old0*sizeof(struct ff_effect));
          for(int _i0 = 0; _i0 < _len_old0; _i0++) {
            old->direction = ((-2 * (next_i()%2)) + 1) * next_i();
        old->replay.length = ((-2 * (next_i()%2)) + 1) * next_i();
        old->replay.delay = ((-2 * (next_i()%2)) + 1) * next_i();
        old->trigger.interval = ((-2 * (next_i()%2)) + 1) * next_i();
        old->trigger.button = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = pidff_needs_set_effect(effect,old);
          printf("%d\n", benchRet); 
          free(effect);
          free(old);

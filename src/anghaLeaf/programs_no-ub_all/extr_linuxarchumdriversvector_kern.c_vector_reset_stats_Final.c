...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_vp0; _i0++) {
            vp->estats.sg_linearized = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.sg_ok = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_flow_control_xoff = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_flow_control_xon = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_kicks = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_restart_queue = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_timeout_count = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.rx_encaps_errors = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_queue_running_average = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.tx_queue_max = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.rx_queue_running_average = ((-2 * (next_i()%2)) + 1) * next_i();
        vp->estats.rx_queue_max = ((-2 * (next_i()%2)) + 1) * next_i();
          }

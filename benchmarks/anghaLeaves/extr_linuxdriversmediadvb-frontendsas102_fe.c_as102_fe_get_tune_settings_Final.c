...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          struct dvb_frontend_tune_settings * settings = (struct dvb_frontend_tune_settings *) malloc(_len_settings0*sizeof(struct dvb_frontend_tune_settings));
          for(int _i0 = 0; _i0 < _len_settings0; _i0++) {
            settings[_i0].min_delay_ms = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = as102_fe_get_tune_settings(fe,settings);
          printf("%d\n", benchRet); 
          free(fe);
          free(settings);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          int idx = 255;
          int _len_src0 = 1;
          struct annotated_source * src = (struct annotated_source *) malloc(_len_src0*sizeof(struct annotated_source));
          for(int _i0 = 0; _i0 < _len_src0; _i0++) {
            src->sizeof_sym_hist = ((-2 * (next_i()%2)) + 1) * next_i();
        src->histograms = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          struct sym_hist * benchRet = annotated_source__histogram(src,idx);
          printf("{{struct}} %p\n", &benchRet); 
          free(src);

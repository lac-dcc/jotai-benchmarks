...
__attribute__((used)) static int video_g_input(struct file *file, void *fh, unsigned int *input)
{
	*input = 0;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          unsigned int * input = (unsigned int *) malloc(_len_input0*sizeof(unsigned int));
          for(int _i0 = 0; _i0 < _len_input0; _i0++) {
            input[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
        
          int benchRet = video_g_input(file,fh,input);
          printf("%d\n", benchRet); 
          free(file);
          free(input);

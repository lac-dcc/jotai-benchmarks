...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          cdev[_i0].devdata = (struct clock_cooling_device *) malloc(_len_cdev__i0__devdata0*sizeof(struct clock_cooling_device));
          for(int _j0 = 0; _j0 < _len_cdev__i0__devdata0; _j0++) {
            cdev[_i0].devdata->clock_state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int _len_state0 = 1;
          ...
          int benchRet = clock_cooling_get_cur_state(cdev,state);
          printf("%d\n", benchRet); 

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_pool0; _i0++) {
            pool->entropy = ((-2 * (next_i()%2)) + 1) * next_i();
        pool->entropy_requested = ((-2 * (next_i()%2)) + 1) * next_i();
        pool->len = ((-2 * (next_i()%2)) + 1) * next_i();
        pool->min_len = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned long benchRet = rand_pool_entropy_available(pool);
          printf("%lu\n", benchRet); 
          free(pool);

...
__attribute__((used)) static int snd_amd7930_info_volume(struct snd_kcontrol *kctl, struct snd_ctl_elem_info *uinfo)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
        ...
        uinfo->type = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          int benchRet = snd_amd7930_info_volume(kctl,uinfo);
          printf("%d\n", benchRet); 
          free(kctl);
          free(uinfo);

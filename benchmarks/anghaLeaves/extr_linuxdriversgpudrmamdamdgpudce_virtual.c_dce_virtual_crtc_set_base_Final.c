...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_old_fb0 = 1;
          struct drm_framebuffer * old_fb = (struct drm_framebuffer *) malloc(_len_old_fb0*sizeof(struct drm_framebuffer));
          for(int _i0 = 0; _i0 < _len_old_fb0; _i0++) {
            old_fb[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = dce_virtual_crtc_set_base(crtc,x,y,old_fb);
          printf("%d\n", benchRet); 
          free(crtc);
          free(old_fb);

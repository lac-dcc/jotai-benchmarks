...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_info0; _i0++) {
            info->rcbaAreap = ((-2 * (next_i()%2)) + 1) * next_i();
        info->rcbaArea = ((-2 * (next_i()%2)) + 1) * next_i();
        info->hpet2bus = ((-2 * (next_i()%2)) + 1) * next_i();
        info->bus2hpet = ((-2 * (next_i()%2)) + 1) * next_i();
        info->hpet2tsc = ((-2 * (next_i()%2)) + 1) * next_i();
        info->tsc2hpet = ((-2 * (next_i()%2)) + 1) * next_i();
        info->hpetCvtn2t = ((-2 * (next_i()%2)) + 1) * next_i();
        info->hpetCvtt2n = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          hpet_get_info(info);
          free(info);

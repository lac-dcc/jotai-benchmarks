...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_vcpu0; _i0++) {
            vcpu->arch.ivpr = ((-2 * (next_i()%2)) + 1) * next_i();
        vcpu->arch.decar = ((-2 * (next_i()%2)) + 1) * next_i();
        vcpu->arch.mcsrr1 = ((-2 * (next_i()%2)) + 1) * next_i();
        vcpu->arch.mcsrr0 = ((-2 * (next_i()%2)) + 1) * next_i();
        vcpu->vcpu_id = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_sregs0 = 1;
          struct kvm_sregs * sregs = (struct kvm_sregs *) malloc(_len_sregs0*sizeof(struct kvm_sregs));
          for(int _i0 = 0; _i0 < _len_sregs0; _i0++) {
            sregs->u.e.ivpr = ((-2 * (next_i()%2)) + 1) * next_i();
        sregs->u.e.decar = ((-2 * (next_i()%2)) + 1) * next_i();
        sregs->u.e.mcsrr1 = ((-2 * (next_i()%2)) + 1) * next_i();
        sregs->u.e.mcsrr0 = ((-2 * (next_i()%2)) + 1) * next_i();
        sregs->u.e.pir = ((-2 * (next_i()%2)) + 1) * next_i();
        sregs->u.e.features = ((-2 * (next_i()%2)) + 1) * next_i();
          }

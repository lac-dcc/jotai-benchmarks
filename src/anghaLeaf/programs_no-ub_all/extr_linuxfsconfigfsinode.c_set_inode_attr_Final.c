...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_inode0; _i0++) {
            inode->i_ctime = ((-2 * (next_i()%2)) + 1) * next_i();
        inode->i_mtime = ((-2 * (next_i()%2)) + 1) * next_i();
        inode->i_atime = ((-2 * (next_i()%2)) + 1) * next_i();
        inode->i_gid = ((-2 * (next_i()%2)) + 1) * next_i();
        inode->i_uid = ((-2 * (next_i()%2)) + 1) * next_i();
        inode->i_mode = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_iattr0 = 1;
          struct iattr * iattr = (struct iattr *) malloc(_len_iattr0*sizeof(struct iattr));
          for(int _i0 = 0; _i0 < _len_iattr0; _i0++) {
            iattr->ia_ctime = ((-2 * (next_i()%2)) + 1) * next_i();
        iattr->ia_mtime = ((-2 * (next_i()%2)) + 1) * next_i();
        iattr->ia_atime = ((-2 * (next_i()%2)) + 1) * next_i();
        iattr->ia_gid = ((-2 * (next_i()%2)) + 1) * next_i();
        iattr->ia_uid = ((-2 * (next_i()%2)) + 1) * next_i();
        iattr->ia_mode = ((-2 * (next_i()%2)) + 1) * next_i();
          }

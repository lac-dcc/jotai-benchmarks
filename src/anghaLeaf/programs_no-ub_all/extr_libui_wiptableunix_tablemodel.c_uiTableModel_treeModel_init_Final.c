...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_iface0; _i0++) {
            iface->iter_parent = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_nth_child = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_n_children = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_has_child = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_children = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_previous = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->iter_next = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_value = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_path = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_iter = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_column_type = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_n_columns = ((-2 * (next_i()%2)) + 1) * next_i();
        iface->get_flags = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          uiTableModel_treeModel_init(iface);
          free(iface);

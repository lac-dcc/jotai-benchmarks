...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p_state0; _i0++) {
            p_state->recv.min_gap_duration = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.min_digit_duration = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.state = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.indication_state_ack = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.indication_state = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.digit_read_pos = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.digit_write_pos = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.current_digit_value = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.current_digit_off_time = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.current_digit_on_time = ((-2 * (next_i()%2)) + 1) * next_i();
        p_state->recv.cycle_counter = ((-2 * (next_i()%2)) + 1) * next_i();
          }

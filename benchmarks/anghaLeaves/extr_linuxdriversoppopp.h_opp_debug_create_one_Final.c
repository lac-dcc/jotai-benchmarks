...
__attribute__((used)) static inline int opp_debug_create_one(struct dev_pm_opp *opp,
...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_opp0 = 1;
          struct dev_pm_opp * opp = (struct dev_pm_opp *) malloc(_len_opp0*sizeof(struct dev_pm_opp));
          for(int _i0 = 0; _i0 < _len_opp0; _i0++) {
            opp[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          ...
          int benchRet = opp_debug_create_one(opp,opp_table);
          printf("%d\n", benchRet); 
          free(opp);
          free(opp_table);

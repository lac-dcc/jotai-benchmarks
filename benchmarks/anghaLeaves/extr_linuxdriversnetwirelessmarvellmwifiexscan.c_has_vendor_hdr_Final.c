...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          long key = 100;
          int _len_ie0 = 1;
          struct ieee_types_vendor_specific * ie = (struct ieee_types_vendor_specific *) malloc(_len_ie0*sizeof(struct ieee_types_vendor_specific));
          for(int _i0 = 0; _i0 < _len_ie0; _i0++) {
            ie[_i0].vend_hdr.element_id = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = has_vendor_hdr(ie,key);
          printf("%d\n", benchRet); 
          free(ie);

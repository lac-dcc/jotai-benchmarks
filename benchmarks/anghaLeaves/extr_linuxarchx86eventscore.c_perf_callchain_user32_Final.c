...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_entry0 = 1;
          struct perf_callchain_entry_ctx * entry = (struct perf_callchain_entry_ctx *) malloc(_len_entry0*sizeof(struct perf_callchain_entry_ctx));
          for(int _i0 = 0; _i0 < _len_entry0; _i0++) {
            entry[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = perf_callchain_user32(regs,entry);
          printf("%d\n", benchRet); 
          free(regs);
          free(entry);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          file[_i0].private_data = (struct ecryptfs_file_info *) malloc(_len_file__i0__private_data0*sizeof(struct ecryptfs_file_info));
          for(int _j0 = 0; _j0 < _len_file__i0__private_data0; _j0++) {
            file[_i0].private_data->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct ecryptfs_file_info * benchRet = ecryptfs_file_to_private(file);
          printf("%d\n", (*benchRet).dummy);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_bdev0; _i0++) {
            bdev->md.meta_dev_idx = ((-2 * (next_i()%2)) + 1) * next_i();
        bdev->md.md_offset = ((-2 * (next_i()%2)) + 1) * next_i();
        bdev->md.bm_offset = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          long benchRet = drbd_md_first_sector(bdev);
          printf("%ld\n", benchRet); 
          free(bdev);

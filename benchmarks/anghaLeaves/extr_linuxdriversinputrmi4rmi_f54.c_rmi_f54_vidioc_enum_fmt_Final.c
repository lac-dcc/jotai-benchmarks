...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_fmt0 = 65025;
          struct v4l2_fmtdesc * fmt = (struct v4l2_fmtdesc *) malloc(_len_fmt0*sizeof(struct v4l2_fmtdesc));
          for(int _i0 = 0; _i0 < _len_fmt0; _i0++) {
              fmt[_i0].type = ((-2 * (next_i()%2)) + 1) * next_i();
          fmt[_i0].index = ((-2 * (next_i()%2)) + 1) * next_i();
          fmt[_i0].pixelformat = ((-2 * (next_i()%2)) + 1) * next_i();
        
          }
        
          int benchRet = rmi_f54_vidioc_enum_fmt(file,priv,fmt);
          printf("%d\n", benchRet); 
          free(file);
          free(fmt);

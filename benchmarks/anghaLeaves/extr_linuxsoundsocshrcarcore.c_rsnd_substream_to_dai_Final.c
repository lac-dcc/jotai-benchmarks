...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          substream[_i0].private_data->cpu_dai = (struct snd_soc_dai *) malloc(_len_substream__i0__private_data_cpu_dai0*sizeof(struct snd_soc_dai));
          for(int _j0 = 0; _j0 < _len_substream__i0__private_data_cpu_dai0; _j0++) {
            substream[_i0].private_data->cpu_dai->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          }
          struct snd_soc_dai * benchRet = rsnd_substream_to_dai(substream);
          printf("%d\n", (*benchRet).dummy);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_settings0; _i0++) {
            settings->btype = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->use_lz77 = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->minmatch = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->nicematch = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->lazymatching = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->custom_context = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->custom_deflate = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->custom_zlib = ((-2 * (next_i()%2)) + 1) * next_i();
        settings->windowsize = ((-2 * (next_i()%2)) + 1) * next_i();
          }

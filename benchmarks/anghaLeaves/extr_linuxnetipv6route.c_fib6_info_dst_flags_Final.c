...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_rt0 = 1;
          struct fib6_info * rt = (struct fib6_info *) malloc(_len_rt0*sizeof(struct fib6_info));
          for(int _i0 = 0; _i0 < _len_rt0; _i0++) {
            rt[_i0].dst_host = ((-2 * (next_i()%2)) + 1) * next_i();
        rt[_i0].dst_nopolicy = ((-2 * (next_i()%2)) + 1) * next_i();
        rt[_i0].dst_nocount = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned short benchRet = fib6_info_dst_flags(rt);
          printf("%hu\n", benchRet); 
          free(rt);

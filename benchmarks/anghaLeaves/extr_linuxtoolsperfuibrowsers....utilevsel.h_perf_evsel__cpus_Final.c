...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_evsel0; _i0++) {
              int _len_evsel__i0__cpus0 = 1;
          evsel[_i0].cpus = (struct cpu_map *) malloc(_len_evsel__i0__cpus0*sizeof(struct cpu_map));
          for(int _j0 = 0; _j0 < _len_evsel__i0__cpus0; _j0++) {
            evsel[_i0].cpus->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          struct cpu_map * benchRet = perf_evsel__cpus(evsel);
          printf("%d\n", (*benchRet).dummy);
          for(int _aux = 0; _aux < _len_evsel0; _aux++) {
          free(evsel[_aux].cpus);
          }
          free(evsel);

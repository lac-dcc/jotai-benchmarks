...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_box0; _i0++) {
            box->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_event0 = 1;
          struct perf_event * event = (struct perf_event *) malloc(_len_event0*sizeof(struct perf_event));
          for(int _i0 = 0; _i0 < _len_event0; _i0++) {
            event->attr.config2 = ((-2 * (next_i()%2)) + 1) * next_i();
        event->attr.config1 = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.config = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.branch_reg.config = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.branch_reg.reg = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.branch_reg.idx = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.extra_reg.config = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.extra_reg.reg = ((-2 * (next_i()%2)) + 1) * next_i();
        event->hw.extra_reg.idx = ((-2 * (next_i()%2)) + 1) * next_i();
          }

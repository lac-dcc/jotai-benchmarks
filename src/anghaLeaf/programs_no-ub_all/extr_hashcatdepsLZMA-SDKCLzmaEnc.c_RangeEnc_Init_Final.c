...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->range = ((-2 * (next_i()%2)) + 1) * next_i();
        p->res = ((-2 * (next_i()%2)) + 1) * next_i();
        p->processed = ((-2 * (next_i()%2)) + 1) * next_i();
        p->bufBase = ((-2 * (next_i()%2)) + 1) * next_i();
        p->buf = ((-2 * (next_i()%2)) + 1) * next_i();
        p->cacheSize = ((-2 * (next_i()%2)) + 1) * next_i();
        p->low = ((-2 * (next_i()%2)) + 1) * next_i();
        p->cache = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          RangeEnc_Init(p);
          free(p);

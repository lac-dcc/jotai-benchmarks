...
struct TYPE_4__ *_allocate_SListHdr(int length) {
...
void _delete_SListHdr(struct TYPE_4__ * head) {
  if(head->Next != NULL) _delete_SListHdr(head->Next);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_4__ * SListHdr = _allocate_SListHdr(1);
          struct TYPE_4__ * benchRet = SimplePopSlist(SListHdr);
          printf("{{struct}} %p\n", &benchRet); 
          _delete_SListHdr(SListHdr);

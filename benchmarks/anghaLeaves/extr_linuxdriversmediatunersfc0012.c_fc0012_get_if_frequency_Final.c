...
__attribute__((used)) static int fc0012_get_if_frequency(struct dvb_frontend *fe, u32 *frequency)
{
	*frequency = 0; /* Zero-IF */
	return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          long * frequency = (long *) malloc(_len_frequency0*sizeof(long));
          for(int _i0 = 0; _i0 < _len_frequency0; _i0++) {
            frequency[_i0] = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = fc0012_get_if_frequency(fe,frequency);
          printf("%d\n", benchRet); 
          free(fe);
          free(frequency);

...
__attribute__((used)) static struct tty_driver *pti_console_device(struct console *c, int *index)
{
	*index = c->index;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          struct tty_driver * benchRet = pti_console_device(c,index);
          printf("{{struct}} %p\n", &benchRet); 
          free(c);
          free(index);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _j0 = 0; _j0 < _len_cptr__i0__hdw0; _j0++) {
            cptr[_i0].hdw->std_mask_avail = ((-2 * (next_i()%2)) + 1) * next_i();
        cptr[_i0].hdw->std_info_cur.def.type_bitmask.valid_bits = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = ctrl_stdavail_set(cptr,m,v);
          printf("%d\n", benchRet); 

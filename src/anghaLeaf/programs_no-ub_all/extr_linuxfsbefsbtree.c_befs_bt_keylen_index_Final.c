...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_node0 = 1;
          struct befs_btree_node * node = (struct befs_btree_node *) malloc(_len_node0*sizeof(struct befs_btree_node));
          for(int _i0 = 0; _i0 < _len_node0; _i0++) {
            node->od_node = ((-2 * (next_i()%2)) + 1) * next_i();
        node->head.all_key_length = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int * benchRet = befs_bt_keylen_index(node);
          printf("{{other_type}} %p\n", &benchRet); 
          free(node);

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_d0 = 1;
          struct device * d = (struct device *) malloc(_len_d0*sizeof(struct device));
          for(int _i0 = 0; _i0 < _len_d0; _i0++) {
            d[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_attr0 = 1;
          struct device_attribute * attr = (struct device_attribute *) malloc(_len_attr0*sizeof(struct device_attribute));
          ...
          int benchRet = il3945_show_channels(d,attr,buf);
          printf("%d\n", benchRet); 
          free(d);
          free(attr);
          free(buf);

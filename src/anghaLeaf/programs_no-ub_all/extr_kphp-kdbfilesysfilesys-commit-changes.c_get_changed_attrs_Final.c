...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_a0; _i0++) {
            a->st_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        a->st_uid = ((-2 * (next_i()%2)) + 1) * next_i();
        a->st_gid = ((-2 * (next_i()%2)) + 1) * next_i();
        a->st_size = ((-2 * (next_i()%2)) + 1) * next_i();
        a->st_mtime = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_b0 = 1;
          struct stat * b = (struct stat *) malloc(_len_b0*sizeof(struct stat));
          for(int _i0 = 0; _i0 < _len_b0; _i0++) {
            b->st_mode = ((-2 * (next_i()%2)) + 1) * next_i();
        b->st_uid = ((-2 * (next_i()%2)) + 1) * next_i();
        b->st_gid = ((-2 * (next_i()%2)) + 1) * next_i();
        b->st_size = ((-2 * (next_i()%2)) + 1) * next_i();
        b->st_mtime = ((-2 * (next_i()%2)) + 1) * next_i();
          }

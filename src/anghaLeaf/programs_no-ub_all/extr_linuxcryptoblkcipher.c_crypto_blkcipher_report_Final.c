...
__attribute__((used)) static int crypto_blkcipher_report(struct sk_buff *skb, struct crypto_alg *alg)
{
	return -ENOSYS;
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_alg0; _i0++) {
            alg->dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = crypto_blkcipher_report(skb,alg);
          printf("%d\n", benchRet); 
          free(skb);
          free(alg);

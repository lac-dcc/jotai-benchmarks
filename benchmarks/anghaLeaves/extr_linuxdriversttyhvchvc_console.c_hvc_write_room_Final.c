...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          tty[_i0].driver_data = (struct hvc_struct *) malloc(_len_tty__i0__driver_data0*sizeof(struct hvc_struct));
          for(int _j0 = 0; _j0 < _len_tty__i0__driver_data0; _j0++) {
            tty[_i0].driver_data->outbuf_size = ((-2 * (next_i()%2)) + 1) * next_i();
        tty[_i0].driver_data->n_outbuf = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = hvc_write_room(tty);
          printf("%d\n", benchRet); 

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_a0; _i0++) {
            a->dst_ip = ((-2 * (next_i()%2)) + 1) * next_i();
        a->src_ip = ((-2 * (next_i()%2)) + 1) * next_i();
        a->dst_port = ((-2 * (next_i()%2)) + 1) * next_i();
        a->src_port = ((-2 * (next_i()%2)) + 1) * next_i();
        a->flow_type = ((-2 * (next_i()%2)) + 1) * next_i();
        a->ip4_proto = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_b0 = 1;
          struct i40e_fdir_filter * b = (struct i40e_fdir_filter *) malloc(_len_b0*sizeof(struct i40e_fdir_filter));
          for(int _i0 = 0; _i0 < _len_b0; _i0++) {
            b->dst_ip = ((-2 * (next_i()%2)) + 1) * next_i();
        b->src_ip = ((-2 * (next_i()%2)) + 1) * next_i();
        b->dst_port = ((-2 * (next_i()%2)) + 1) * next_i();
        b->src_port = ((-2 * (next_i()%2)) + 1) * next_i();
        b->flow_type = ((-2 * (next_i()%2)) + 1) * next_i();
        b->ip4_proto = ((-2 * (next_i()%2)) + 1) * next_i();
          }

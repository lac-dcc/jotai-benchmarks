...
struct TYPE_4__ *_allocate_message(int length) {
...
void _delete_message(struct TYPE_4__ * head) {
  if(head->next != NULL) _delete_message(head->next);
  if(head->prev != NULL) _delete_message(head->prev);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_4__ * message = _allocate_message(1);

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_utask0; _i0++) {
            utask->xol_vaddr = ((-2 * (next_i()%2)) + 1) * next_i();
        utask->vaddr = ((-2 * (next_i()%2)) + 1) * next_i();
        utask->autask.saved_tnpc = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_regs0 = 1;
          struct pt_regs * regs = (struct pt_regs *) malloc(_len_regs0*sizeof(struct pt_regs));
          for(int _i0 = 0; _i0 < _len_regs0; _i0++) {
            regs->tnpc = ((-2 * (next_i()%2)) + 1) * next_i();
        regs->tpc = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_dst0; _i0++) {
            dst->outbytes = ((-2 * (next_i()%2)) + 1) * next_i();
        dst->inbytes = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int _len_src0 = 1;
          struct ip_vs_kstats * src = (struct ip_vs_kstats *) malloc(_len_src0*sizeof(struct ip_vs_kstats));
          for(int _i0 = 0; _i0 < _len_src0; _i0++) {
            src->outbps = ((-2 * (next_i()%2)) + 1) * next_i();
        src->inbps = ((-2 * (next_i()%2)) + 1) * next_i();
        src->outpps = ((-2 * (next_i()%2)) + 1) * next_i();
        src->inpps = ((-2 * (next_i()%2)) + 1) * next_i();
        src->cps = ((-2 * (next_i()%2)) + 1) * next_i();
        src->outbytes = ((-2 * (next_i()%2)) + 1) * next_i();
        src->inbytes = ((-2 * (next_i()%2)) + 1) * next_i();
        src->outpkts = ((-2 * (next_i()%2)) + 1) * next_i();
        src->inpkts = ((-2 * (next_i()%2)) + 1) * next_i();
        src->conns = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_cfg0; _i0++) {
            cfg->tx_pad_crc = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->ptp_tsu_en = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->ptp_exception_en = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->maximum_frame = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->back_to_back_ipg = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->min_ifg_enforcement = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->non_back_to_back_ipg2 = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->non_back_to_back_ipg1 = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->tx_pause_time_extd = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->preamble_len = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->rx_prepend = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->tx_pause_time = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->halfdup_coll_window = ((-2 * (next_i()%2)) + 1) * next_i();
        cfg->halfdup_retransmit = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          set_dflts(cfg);
          free(cfg);

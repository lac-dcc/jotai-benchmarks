...
struct tl_token *_allocate_L(int length) {
  struct tl_token *head = (struct tl_token *)malloc(sizeof(struct tl_token));

  head->next = NULL;

  struct tl_token *walker = head;
...
void _delete_L(struct tl_token * head) {
  if(head->next != NULL) _delete_L(head->next);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct tl_token * L = _allocate_L(1);

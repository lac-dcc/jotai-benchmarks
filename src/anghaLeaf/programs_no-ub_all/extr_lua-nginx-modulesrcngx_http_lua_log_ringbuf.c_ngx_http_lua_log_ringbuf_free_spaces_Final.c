...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_rb0; _i0++) {
            rb->count = ((-2 * (next_i()%2)) + 1) * next_i();
        rb->size = ((-2 * (next_i()%2)) + 1) * next_i();
        rb->tail = ((-2 * (next_i()%2)) + 1) * next_i();
        rb->head = ((-2 * (next_i()%2)) + 1) * next_i();
        rb->data = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          unsigned long benchRet = ngx_http_lua_log_ringbuf_free_spaces(rb);
          printf("%lu\n", benchRet); 
          free(rb);

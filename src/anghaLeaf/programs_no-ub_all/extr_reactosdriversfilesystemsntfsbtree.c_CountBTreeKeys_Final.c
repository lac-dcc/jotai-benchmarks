...
struct TYPE_3__ *_allocate_FirstKey(int length) {
...
void _delete_FirstKey(struct TYPE_3__ * head) {
  if(head->NextKey != NULL) _delete_FirstKey(head->NextKey);
  free(head);
}
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          struct TYPE_3__ * FirstKey = _allocate_FirstKey(1);
          long benchRet = CountBTreeKeys(FirstKey);
          printf("%ld\n", benchRet); 
          _delete_FirstKey(FirstKey);

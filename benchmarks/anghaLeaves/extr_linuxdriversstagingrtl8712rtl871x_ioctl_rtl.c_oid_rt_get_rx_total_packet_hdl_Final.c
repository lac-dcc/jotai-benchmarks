    ...
    case 0:
    {
        ...
        poid_par_priv[_i0].information_buf = ((-2 * (next_i()%2)) + 1) * next_i();
          ...
          for(int _j0 = 0; _j0 < _len_poid_par_priv__i0__adapter_context0; _j0++) {
            poid_par_priv[_i0].adapter_context->recvpriv.rx_drop = ((-2 * (next_i()%2)) + 1) * next_i();
        poid_par_priv[_i0].adapter_context->recvpriv.rx_pkts = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          }
          int benchRet = oid_rt_get_rx_total_packet_hdl(poid_par_priv);
          printf("%d\n", benchRet); 

...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_graph0; _i0++) {
            graph[_i0].position = ((-2 * (next_i()%2)) + 1) * next_i();
        graph[_i0].row.size = ((-2 * (next_i()%2)) + 1) * next_i();
        graph[_i0].parents.size = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = graph_needs_expansion(graph);
          printf("%d\n", benchRet); 
          free(graph);

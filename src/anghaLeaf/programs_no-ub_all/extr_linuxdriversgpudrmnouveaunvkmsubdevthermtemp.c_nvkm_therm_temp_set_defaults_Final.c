...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_therm0; _i0++) {
            therm->bios_sensor.thrs_shutdown.temp = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_shutdown.hysteresis = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_critical.temp = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_critical.hysteresis = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_down_clock.temp = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_down_clock.hysteresis = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_fan_boost.temp = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.thrs_fan_boost.hysteresis = ((-2 * (next_i()%2)) + 1) * next_i();
        therm->bios_sensor.offset_constant = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_entry0 = 1;
          struct dccp_feat_entry * entry = (struct dccp_feat_entry *) malloc(_len_entry0*sizeof(struct dccp_feat_entry));
          for(int _i0 = 0; _i0 < _len_entry0; _i0++) {
            entry[_i0].is_local = ((-2 * (next_i()%2)) + 1) * next_i();
        entry[_i0].needs_confirm = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = dccp_feat_genopt(entry);
          printf("%d\n", benchRet); 
          free(entry);

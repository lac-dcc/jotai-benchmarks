...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_p0; _i0++) {
            p->outDataWritten = ((-2 * (next_i()%2)) + 1) * next_i();
        p->decodeOnlyOneBlock = ((-2 * (next_i()%2)) + 1) * next_i();
        p->padSize = ((-2 * (next_i()%2)) + 1) * next_i();
        p->numTotalBlocks = ((-2 * (next_i()%2)) + 1) * next_i();
        p->numFinishedStreams = ((-2 * (next_i()%2)) + 1) * next_i();
        p->numStartedStreams = ((-2 * (next_i()%2)) + 1) * next_i();
        p->pos = ((-2 * (next_i()%2)) + 1) * next_i();
        p->state = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          XzUnpacker_Init(p);
          free(p);

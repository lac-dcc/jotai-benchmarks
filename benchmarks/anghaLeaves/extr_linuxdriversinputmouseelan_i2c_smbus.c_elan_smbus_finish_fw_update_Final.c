...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          int _len_fw_completion0 = 1;
          struct completion * fw_completion = (struct completion *) malloc(_len_fw_completion0*sizeof(struct completion));
          for(int _i0 = 0; _i0 < _len_fw_completion0; _i0++) {
            fw_completion[_i0].dummy = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = elan_smbus_finish_fw_update(client,fw_completion);
          printf("%d\n", benchRet); 
          free(client);
          free(fw_completion);

...
__attribute__((used)) static inline bool page_in_range(struct ashmem_range *range, size_t page)
{
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_range0; _i0++) {
            range->pgstart = ((-2 * (next_i()%2)) + 1) * next_i();
        range->pgend = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = page_in_range(range,page);
          printf("%d\n", benchRet); 
          free(range);

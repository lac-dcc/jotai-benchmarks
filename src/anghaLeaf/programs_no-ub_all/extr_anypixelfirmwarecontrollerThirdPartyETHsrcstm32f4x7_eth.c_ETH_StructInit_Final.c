...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          for(int _i0 = 0; _i0 < _len_ETH_InitStruct0; _i0++) {
            ETH_InitStruct->ETH_HashTableHigh = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_HashTableLow = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_PauseTime = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_VLANTagIdentifier = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_DescriptorSkipLength = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_DMAArbitration = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_TxDMABurstLength = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_RxDMABurstLength = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_FixedBurst = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_AddressAlignedBeats = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_SecondFrameOperate = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ReceiveThresholdControl = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ForwardUndersizedGoodFrames = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ForwardErrorFrames = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_TransmitThresholdControl = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_TransmitStoreForward = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_FlushReceivedFrame = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ReceiveStoreForward = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_VLANTagComparison = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_TransmitFlowControl = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ReceiveFlowControl = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_UnicastPauseFrameDetect = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_PauseLowThreshold = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ZeroQuantaPause = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_UnicastFramesFilter = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_MulticastFramesFilter = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_PromiscuousMode = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_DestinationAddrFilter = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_BroadcastFramesReception = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_PassControlFrames = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_SourceAddrFilter = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ReceiveAll = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_DeferralCheck = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_BackOffLimit = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_AutomaticPadCRCStrip = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_RetryTransmission = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ChecksumOffload = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_Mode = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_LoopbackMode = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_ReceiveOwn = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_Speed = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_CarrierSense = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_InterFrameGap = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_Jabber = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_Watchdog = ((-2 * (next_i()%2)) + 1) * next_i();
        ETH_InitStruct->ETH_AutoNegotiation = ((-2 * (next_i()%2)) + 1) * next_i();
          }

...
__attribute__((used)) static unsigned char analogix_dp_get_adjust_request_pre_emphasis(
					u8 adjust_request[2],
					int lane)
{
	int shift = (lane & 1) * 4;
...
int main(int argc, char *argv[]) {
    ...
    case 0:
    {
          ...
          unsigned char benchRet = analogix_dp_get_adjust_request_pre_emphasis(adjust_request,lane);
          printf("%c\n", (benchRet %26) + 'a'); 
          free(adjust_request);

...
int main(int argc, char *argv[]) {
    ...
    {
          int _len_db0 = 1;
          struct txdb * db = (struct txdb *) malloc(_len_db0*sizeof(struct txdb));
          for(int _i0 = 0; _i0 < _len_db0; _i0++) {
            db[_i0].wptr = ((-2 * (next_i()%2)) + 1) * next_i();
        db[_i0].rptr = ((-2 * (next_i()%2)) + 1) * next_i();
        db[_i0].size = ((-2 * (next_i()%2)) + 1) * next_i();
          }
          int benchRet = bdx_tx_db_size(db);
          printf("%d\n", benchRet); 
          free(db);
